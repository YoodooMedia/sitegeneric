function miniGraph(target,data) {
	this.data={
		...
		lines:[
			...
			{colour:'#00ddff',width:1},
			{colour:'#00ffdd',width:1}
		],
		padding:5,
		data:[
	...
	this.render=function() {
		if (this.container!==null && this.data.data.length>1) {
			if (this.data.data[0].length==undefined) {
				var tmp=[];
				while(this.data.data.length>0) {
					tmp.push(this.data.data.shift());
				}
				this.data.data=[tmp];
			}
			this.dots=[];
			...
			if (this.data.max!==null) this.range.y[1]=this.data.max;
			if (this.data.min!==null) this.range.y[0]=this.data.min;
			for(var d=0;d<this.data.data.length;d++) {
				for(var dd=0;dd<this.data.data[d].length;dd++) {
					if (this.data.data[d][dd].x<this.range.x[0]) this.range.x[0]=this.data.data[d][dd].x;
					if (this.data.data[d][dd].x>this.range.x[1]) this.range.x[1]=this.data.data[d][dd].x;
					if (this.data.min===null && this.data.data[d][dd].y<this.range.y[0]) this.range.y[0]=this.data.data[d][dd].y;
					if (this.data.max===null && this.data.data[d][dd].y>this.range.y[1]) this.range.y[1]=this.data.data[d][dd].y;
				}
			}
			...
			//for(var d=0;d<this.data.data.length;d++) {
					var pos={x:0,y:0};
					pos.x=(this.data.data[d][dd].x-this.range.x[0])*this.range.dx;
					pos.y=(this.range.y[1]-this.data.data[d][dd].y)*this.range.dy;
					pos.x+=this.data.padding;
					pos.y+=this.data.padding;
					dotRow.push([this.data.data[d][dd].legend,pos]);
				}
	...
	this.destroy=function() {
		$(this.bubble).remove();
		this.removeClosest();
		$(this.containerOverlay).unbind("mousemove").unbind("mouseleave").unbind("mouseenter");
	};
	this.closestDot=function(pos) {
		var nearestH=10000;
		var closest=null;
		for(var d=0;d<this.dots.length;d++) {
			for(var dd=0;dd<this.dots[d].length;dd++) {
				var h=Math.sqrt(Math.pow(pos.x-this.dots[d][dd][1].x,2)+Math.pow(pos.y-this.dots[d][dd][1].y,2));
				if (h<nearestH) {
					nearestH=h;
					closest=this.dots[d][dd];
				}
			}
		}
		return closest;
	...
	this.renderPoints=function() {
		switch(this.type) {
			case "svg":
				for(var d=0;d<this.dots.length;d++) {
					for(var dd=1;dd<this.dots[d].length;dd++) {
						var e=document.createElementNS("http://www.w3.org/2000/svg", "path");
						var str='M'+this.dots[d][dd-1][1].x+' '+this.dots[d][dd-1][1].y+' L'+this.dots[d][dd][1].x+' '+this.dots[d][dd][1].y+' Z';
						e.setAttribute("d",str);
						e.setAttribute("stroke",this.data.lines[d].colour);
						e.setAttribute("stroke-width",this.data.lines[d].width);
				...
				for(var d=0;d<this.dots.length;d++) {
					for(var dd=0;dd<this.dots[d].length;dd++) {
						var e=document.createElementNS("http://www.w3.org/2000/svg", "circle");
						e.setAttribute("cx",this.dots[d][dd][1].x);
						e.setAttribute("cy",this.dots[d][dd][1].y);
						e.setAttribute("r",3);
			...
			case "canvas":
				for(var d=0;d<this.dots.length;d++) {
					this.context.lineWidth=this.data.lines[d].width;
					//this.context.strokeStyle='rgb(0,0,0)';
					this.context.strokeStyle=this.data.lines[d].colour;
					this.context.beginPath();
					this.context.moveTo(this.dots[d][0][1].x,this.dots[d][0][1].y);
					for(var dd=1;dd<this.dots[d].length;dd++) {
						this.context.lineTo(this.dots[d][dd][1].x,this.dots[d][dd][1].y);
					}
					this.context.stroke();
				}
				for(var d=0;d<this.dots.length;d++) {
					...
					for(var dd=0;dd<this.dots[d].length;dd++) {
						this.context.beginPath();
						this.context.arc(this.dots[d][dd][1].x,this.dots[d][dd][1].y,3,0,2*Math.PI,false);
