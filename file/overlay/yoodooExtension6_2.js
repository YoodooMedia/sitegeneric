		...
		var standard={
			...
			loadFields: function() {
				...
				if (yoodoo.dooit!==undefined) {
					...
					if (this.globalkey !== null && this.globalkey !== undefined && typeof(ui_questions)!="undefined" && array_of_fields[this.globalkey][1]!="")  try{eval('ui_questions.schema='+array_of_fields[this.globalkey][1]+';');}catch(e){
						yoodoo.errorLog(e);
					};
					if (array_of_default_fields.length>1) {
						...
						if (this.key !== null && this.key !== undefined && typeof(ui_questions)!="undefined" && array_of_fields[this.key][1]!="") try{eval('ui_questions.value='+array_of_fields[this.key][1]+';');}catch(e){yoodoo.errorLog(e);ui_questions.value=array_of_fields[this.key][1];};
					}
			...
			//	var require=$(this.containers.container).find('.stillRequired').get().length>0;
				return {
					canSave:true,
					canComplete:true,//!require,   //fixme
					hilite:true
				};
			},
			presave: function() {},
	...
	if (this.bookcase != undefined) {
		if (this.bookcase.tags != undefined) {
			for (var t = 0; t < this.bookcase.tags.length; t++) {
				if (isNaN(tag) && this.bookcase.tags[t].name == tag) {
					return true;
				}else if (this.bookcase.tags[t].id == tag) {
					return true;
				}
			}
		}
...
yoodoo.togo=function(when) {
	var today=new Date().getTime()/1000;
	var secs=when.getTime()/1000;
	var left=Math.round(secs-today);
	if (left>0) {
		if (left<120) {
			return left+' '+yoodoo.w("second"+(left==1?'':'s'));
		}else{
			left=Math.floor(left/60);
			if (left<120) {
				return left+' '+yoodoo.w("minute"+(left==1?'':'s'));
			}else{
				left=Math.floor(left/60);
				if (left<120) {
					return left+' '+yoodoo.w("hour"+(left==1?'':'s'));
				}else{
					left=Math.floor(left/24);
					if (left<14) {
						return left+' '+yoodoo.w("day"+(left==1?'':'s'));
					}else{
						left=Math.floor(left/7);
						return left+' '+yoodoo.w("week"+(left==1?'':'s'));
					}
				}
			}
...
yoodoo.ago = function(d) {
	...
	if (arguments.length > 1)
		brief = arguments[0];
	var inFuture=d>new Date();
	//console.log(inFuture);
	var today = new Date().getTime() / 1000;
	var ts = d.getTime() / 1000;
	var dt = today - ts;
	if (inFuture)  dt = ts - today;
	var minute = 60;
	var hour = 60 * minute;
	var day = hour * 24;
	var week = 7 * day;
	...
	if (brief && !inFuture) {
		...
		if (q == 1)
			return inFuture?yoodoo.w("ina" + u + ""):yoodoo.w("a" + u + "ago");
		return inFuture?yoodoo.w("innum" + u + "s",{n:q}):yoodoo.w("n" + u + "sago",{n:q});
	}
};
...
yoodoo.replyValues = function(reply) {
	...
	yoodoo.user.username = reply.username;
	...
	yoodoo.user.managerType = (reply.managerType===undefined)?null:reply.managerType;
	...
	showMeta : function(list) {
		if (this.container !== undefined && this.container !== null) {
			...
			for (var k in this.lists.meta) {
				if (this.lists.meta[k]!==undefined && list[k] === undefined) {
...
yoodoo.jsError = function(err) {
	if (typeof(console) != 'undefined' && typeof(console.log) != 'undefined') console.log(err);
...
yoodoo.sendPost = function(u, f) {
	if (!yoodoo.sendingPost) {
		...
		if (f.cmd !== undefined) {
			if (f.cmd=='objects') {
				if (f.cmdaction=="load" && (f.objectNames instanceof Array || f.objectIds instanceof Array)) {
					var q=[];
					if (f.objectNames instanceof Array && f.objectNames.length>0) q.push(f.objectNames.join(','));
					if (f.objectIds instanceof Array && f.objectIds.length>0) q.push(f.objectIds.join(','));
					yoodoo.debuggerz.sendpost(f.cmd+' '+f.cmdaction+' ['+q.join(',')+']');
				}else if (f.cmdaction=="view") {
					yoodoo.debuggerz.sendpost(f.cmd+' '+f.cmdaction+' ['+f.viewId+']');
				}else if (f.objectId>0) {
					yoodoo.debuggerz.sendpost(f.cmd+' '+f.cmdaction+' in '+f.objectId);
				}else{
					yoodoo.debuggerz.sendpost(f.cmd+' '+f.cmdaction);
				}
			}else{
		...
		f.language = yoodoo.words.language;
...
yoodoo.clipboard={
	...
	recover:function() {
		if (typeof(localStorage)=="object") {
			if (localStorage['yoodooClipboard']!==undefined) {
				try{
					this.data=$.parseJSON(localStorage['yoodooClipboard']);
				}catch(e) {}
			}
		}
	},
	clear:function() {
		this.data={};
		localStorage['yoodooClipboard']='';
	},
	add:function(type,title,data,description) {
		...
		localStorage['yoodooClipboard']=JSON.stringify(this.data);
		yoodoo.alert("&lsquo;"+title+'&rsquo; add to the clipboard', [], 4);
	},
	list:function(type) {
		if (this.data[type]!==undefined) return this.data[type];
		return {};
	},
	erase:function(obj,type) {
		if (this.data[type]!==undefined && this.data[type][obj.title]!==undefined) {
			delete this.data[type][obj.title];
		}
		localStorage['yoodooClipboard']=JSON.stringify(this.data);
	},
	selector:function(callback,type) {
		this.recover();
		var l=$(yoodoo.e("div")).addClass("clipboardList")
		var d=$(yoodoo.e("div")).addClass("clipboard").append(
			$(yoodoo.e("div")).addClass("clipboardArena").append($(yoodoo.e("h2")).html("Clipboard")).append(l)
		);
		var arr=this.list(type);
		var op=new yoodoo.ui.dialog({
			html:d,
			blockoutClickClose:true,
			closeButton:true
		});
		var titles=[];
		for(var title in arr) titles.push([title,arr[title].c]);
		titles.sort(function(a,b) {
			return b[1]-a[1];
		});
		for(var t in titles) {
			var title=titles[t][0];
			(function() {
				var obj=arr[title];
				l.append(
					$(yoodoo.e("button")).attr("type","button").html(title).append(
						$(yoodoo.e("div")).html(obj.m)
					).prepend(
						$(yoodoo.e("span")).html(yoodoo.ago(new Date(obj.c))).append(
							$(yoodoo.e("button")).attr("type","button").append(yoodoo.icons.get('close',12,12)).click(function(e) {
								e.preventDefault();
								yoodoo.clipboard.erase(obj,type);
						...
						if (clicked.tagName!="BUTTON") clicked=yoodoo.parentOfType(clicked,["BUTTON"]);
						if (clicked!==this) {
...
window.onerror = function(error, url, line, colno, errorObject) {
	//if (typeof(console)!="undefined" && console.log!==undefined) console.log("Caught error");
	var stack='';
	if (typeof(errorObject)!="undefined" && typeof(errorObject.stack)=="string") stack=errorObject.stack.substring(0,512);
	if (yoodoo.loginCode!="") 
	yoodoo.jsError({
		error : error,
		url : url,
