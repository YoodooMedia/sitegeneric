...
scrapbooker = {
	...
	init : function(params) {
		...
		this.showingPage = 0;
		this.mobile = yoodoo.isApp;
		this.mobile = false;
		this.container = (params.container == undefined) ? this.container : params.container;
		if (this.container !== null) {
			this.createDesktop();
			if (params.xml !== undefined)
				this.setXML(params.xml);

		}
		libraryOptions = {
			buttons : {
				'library' : {
					name : __('Add an image'),
					type : 'libraryButton'
				}
			},
			iconFiles : {
				'library' : yoodoo.option.baseUrl + 'utilities/uploader/nicLibrary.png'
			}
		};
	...
	dispose : function() {
		$(yoodoo.widget).css({
			width : yoodoo.option.width,
			height : yoodoo.option.height
		});
	...
	setXML : function(xml) {
		...
		for (var p = 0; p < this.pages.mine.length; p++) {
			if (this.pages.mine[p].details.displayed == true)
				this.showingPage = p;
		}
		...
		if (!yoodoo.isApp || true)
			setTimeout(function() {scrapbooker.managePageTabs();},200);
	...
	destroy : function() {
		...
		yoodoo.scrapbookFiles.removeFiles();
	...
	lastScrollIdleTime : 500,
	lastScrollTimer : null,
	revealToolbar : function() {
		//$(scrapbooker.scrapbookPages).css({top:window.scrollY,left:window.scrollX,width:window.innerWidth}).fadeIn();
	},
	createDesktop : function() {
		if (yoodoo.isApp && false) {
			this.placeholder = document.createElement("div");
			$(this.placeholder).addClass("mobile_scrapbook");
			this.desktop = document.createElement("div");
			$(this.desktop).addClass("scrapbookDesktop");
			this.scrapbookPages = document.createElement("div");
			$(this.scrapbookPages).addClass("scrapbookToolbar");
			// "+yoodoo.class_prefix+"_overlay");
			//if (yoodooPlaya!=undefined) {
			$(this.scrapbookPages).addClass(yoodoo.class_prefix + "_toolbar").css({
				position : 'fixed',
				left : 0,
				right : 0,
				top : -1,
			...
			//}
			this.placeholder.appendChild(this.desktop);
			this.placeholder.appendChild(this.scrapbookPages);
			yoodoo.widget.appendChild(this.placeholder);
			$(yoodoo.widget).css({
				width : 'auto',
				height : 'auto'
			});
			...
			this.desktopWindow.width = yoodoo.option.width * 1;
			this.desktopWindow.height = yoodoo.option.height * 1;
			$(this.placeholder).css("min-width", this.desktopWindow.width + "px");
			$(this.placeholder).css("min-height", this.desktopWindow.height + "px");
			$(this.desktop).css("min-width", this.desktopWindow.width + "px");
			$(this.desktop).css("min-height", this.desktopWindow.height + "px");
			/*$(window).bind("orientationchange",function() {
			 scrapbooker.lastScrollTimer=setTimeout('scrapbooker.revealToolbar();',scrapbooker.lastScrollIdleTime);
			 });
			 $(window).bind("scroll",function() {
			 $(scrapbooker.scrapbookPages).css({display:"none"});
			 clearTimeout(scrapbooker.lastScrollTimer);
			 scrapbooker.lastScrollTimer=setTimeout('scrapbooker.revealToolbar();',scrapbooker.lastScrollIdleTime);
			 });
			 $(this.desktop).bind("touchstart",function() {
			 $(scrapbooker.scrapbookPages).css({display:"none"});
			 clearTimeout(scrapbooker.lastScrollTimer);
			 scrapbooker.lastScrollTimer=setTimeout('scrapbooker.revealToolbar();',scrapbooker.lastScrollIdleTime);
			 });*/
			$(this.scrapbookPages).bind("touchstart", function(e) {
				//alert(yoodoo.option.width+","+$(this).width()+","+this.style.width+","+scrapbooker.desktopWindow.width);
				if (e.target == this)
					e.preventDefault();
			});
		} else {
	...
	drawPages : function() {
		...
		if (yoodoo.isApp && false) {
			...
			while (this.scrapbookPages.childNodes.length > 0)
			this.scrapbookPages.removeChild(this.scrapbookPages.childNodes[0]);
			var tabPages = [];
			var labels = [];
			var firstTitle = '';
			if (this.showingGroups) {
				...
				for (var p = 0; p < this.pages[this.whos].length; p++) {
					if (this.pages[this.whos][p].details.type != "group") {
						pc++;
						if (firstTitle == "")
							firstTitle = this.pages[this.whos][p].details.title;
					}
				}
				if (pc > 1) {
					labels.push("Pages...");
			...
			$(ds).addClass('pageTabBlock').css({
				'white-space' : 'nowrap',
				height : 30
			});
			if (yoodoo.isApp) {
				$(ds).append(this.pageSelector());
				d.appendChild(ds);
			} else {
				var dd = document.createElement("div");
				dd.appendChild(ds);
				$(dd).addClass('pageTabBlockScroller').css({
					overflow : 'hidden'
				});
				var moreLeft = document.createElement("div");
				$(moreLeft).addClass("moreLeft").css({
					display : "block"
				}).bind('click', function() {
					scrapbooker.scrollTabs(-1);
				});
				var moreRight = document.createElement("div");
				$(moreRight).addClass("moreRight").css({
					display : "block"
				}).bind('click', function() {
					scrapbooker.scrollTabs(1);
				});
				...
				for (var p = 0; p < tabPages.length; p++) {
					...
					$(a).addClass("tab");
					a.title = labels[p];
					if (tabPages[p] == this.showingPage)
						$(a).addClass("on");
	...
	drawToolbar : function() {
		...
		for (var p = 0; p < this.pages[this.whos].length; p++) {
			...
			if (this.pages[this.whos][p].details.type == "group") {
				opt.text = "Group: " + this.pages[this.whos][p].details.title;
			} else {
				opt.text = this.pages[this.whos][p].details.title;
			}
			opt.value = p;
			if (p == this.showingPage)
				opt.selected = true;
			sel.appendChild(opt);
		}
		$(sel).bind("change", function() {
			scrapbooker.setPage(this.value);
		});
		var selector = document.createElement("div");
		this.scrapbookPages.appendChild(but);
		selector.appendChild(sel);
		this.scrapbookPages.appendChild(selector);
		//this.revealToolbar();
	},
	...
	managePageTabs : function() {
		...
		if (pa.length > 0) {
			var sw = $(pa[pa.length - 1]).offset().left + $(pa[pa.length - 1]).outerWidth(true) - $(pa[0]).offset().left + 12;
			$(this.scrapbookPages).find(".pageTabBlock").css({
				width : sw
			});
			for (var i = 0; i < a.length; i++)
				used += $(a[i]).outerWidth(true);
			var available = w - used;
			$(this.scrapbookPages).find(">div").css({
				'max-width' : available
			});
			if ($(this.scrapbookPages).find(".pageTabBlock").width() > $(this.scrapbookPages).find(">div").width()) {
				$(this.scrapbookPages).find(".moreLeft,.moreRight").css({
					display : "block"
				});
	...
	setPage : function(i) {
		...
		this.sendingPost();
		if (i >= this.pages[this.whos].length)
			i = 0;
		this.showingPage = i;
	...
	updateBin : function() {
		...
		//if (this.mobile) alert('<img src="'+yoodoo.option.baseUrl+'uploads/sitegeneric/file/overlay/addons/recycle.png" />');
		$(this.scrapbookPages).find('.bin .recyclebin').html('<span></span>' + this.recycle.length);
		if (this.recycle.length == 0)
			this.hideBin();
	},
	hideBin : function() {
	...
	purgeBin : function() {
		...
		if (scrapbooker.pages.mine[scrapbooker.showingPage].details.type == "scrapbook") {
			yoodoo.sendPost(null, {
				...
				context:scrapbooker,
				method : 'purge',
				page : scrapbooker.pages.mine[scrapbooker.showingPage].details.id,
	...
	addNewNoteButton : function() {
		...
		if (this.whos == "mine") {
			if ($(scrapbooker.scrapbookPages).find(".bin").get().length == 0) {
				var a = document.createElement("a");
				$(a).html("<span class='l'>&nbsp;</span><span class='c'><span class='recyclebin'><span></span>" + this.recycle.length + "</span></span><span class='r'>&nbsp;</span>");
				$(a).addClass("tab");
				$(a).addClass("bin");
	...
	clearAndRedraw : function() {
		...
		while (scrapbooker.desktop.childNodes.length > 0)
		$(scrapbooker.desktop.childNodes[0]).remove();
	...
	parseElements : function(xml) {
		...
		for (var i = 0; i < xml.childNodes.length; i++) {
			if (xml.childNodes[i].nodeName == "scrapbook")
				top = xml.childNodes[i].childNodes;
		}
		if (top != null) {
	...
	createItems : function() {
		if (this.pages[this.whos][this.showingPage].users == undefined)
			this.showingUsers = false;
		if (this.pages[this.whos][this.showingPage].details.type != "group")
			this.showingUsers = false;
		...
		$(this.scrapbookPages).find('.groupUserToggle').remove();
		if (userButton) {
			...
			var bin = $(this.scrapbookPages).find(".bin").get();
			if (bin.length == 0) {
				this.scrapbookPages.appendChild(but);
			} else {
				this.scrapbookPages.insertBefore(but, bin[0]);
			}
		}
		...
		if (this.pages[this.whos][this.showingPage].details.autoPosition == undefined) {
			items.sort(function(a, b) {
				return a.details.location.z - b.details.location.z;
			});
		}
		var elements = [];
		if (items.length == 0) {
		...
		if (this.whos == "mine") {
			if (this.pages[this.whos][this.showingPage].details.type == "scrapbook") {
				$(elements).bind('contextmenu', function(e) {
					e.preventDefault();
					if (!$(this).hasClass("nocontext"))
						scrapbooker.contextmenu(e, this);
				});
			}
		}
		$(elements).find('button.public').bind("click", function(e) {
			...
			if (scrapbooker.mobile) {
				$(source.blockout).css({
					width : scrapbooker.desktopWindow.width,
					height : scrapbooker.desktopWindow.height
				});
			}
			...
			for (var g = 0; g < scrapbooker.pages.mine.length; g++) {
				if (scrapbooker.pages.mine[g].details.type == "group") {
					...
					for (var mg = 0; mg < groups.length; mg++) {
						if (groups[mg].details.id == scrapbooker.pages.mine[g].details.id)
							on = true;
					}
			...
			$(source.shareBox).find("input").bind('change', function() {
				$(this.parentNode.parentNode).find("button").css({
					display : "block"
				});
			});
		...
		$(elements).find('.tools button.remove').bind("click", function() {
			scrapbooker.deleteItem(this.parentNode.parentNode.parentNode);
		});
		$(elements).find('.tools button.edit').bind("click", function() {
			scrapbooker.editItem(this.parentNode.parentNode.parentNode);
		});
		$(elements).find('.tools button.grab').bind("click", function() {
			this.parentNode.parentNode.parentNode.sourceObject.grabItem();
		});
		$(elements).bind("click", function(e) {
			if ((e.target == this || e.target.parentNode == this || $(e.target).hasClass("description")) && $(e.target).data('events') == undefined) {
				$('.scrapbookTopItem').removeClass("scrapbookTopItem");
				this.sourceObject.toTop();
				$(this).addClass("scrapbookTopItem");
				if (!yoodoo.isApp || true)
		...
		$(elements).find(".comment").bind("click", function() {
			$(this).find(".commentTools").slideDown(function() {
				...
				if ($(com).hasClass("scrapbookItem"))
					com.sourceObject.commentSlider();
			});
		});
		...
		if (yoodoo.is_touch) {
			$(elements).find('.scrapbookPin.movable').bind("touchstart", function(e) {
				//alert(e.originalEvent.touches[0].pageX);
				if (e.target == this) {
					e.preventDefault();
					var offset = {
						x : $(this.parentNode).position().left - e.originalEvent.touches[0].pageX,
						y : $(this.parentNode).position().top - e.originalEvent.touches[0].pageY
					};
					scrapbooker.moving = {
					...
					$(window).bind("touchmove", function(e) {
						e.preventDefault();
						var x = e.originalEvent.touches[0].pageX + scrapbooker.moving.offset.x;
						var y = e.originalEvent.touches[0].pageY + scrapbooker.moving.offset.y;
					...
					$(window).bind("touchend", function(e) {
						$(window).unbind("touchmove");
						$(window).unbind("touchend");
						var x = e.originalEvent.changedTouches[0].pageX + scrapbooker.moving.offset.x;
						var y = e.originalEvent.changedTouches[0].pageY + scrapbooker.moving.offset.y;
						scrapbooker.moving.object.sourceObject.details.location.x = x;
						scrapbooker.moving.object.sourceObject.details.location.y = y;
						$(scrapbooker.moving.object).css("left", x + "px");
						$(scrapbooker.moving.object).css("top", y + "px");
			...
			$(elements).find('.scrapbookPin.movable').bind("mousedown", function(e) {
				...
				$(document).bind("mouseup.scrapup mouseleave.scrapup", function(e) {
					var x = e.pageX + scrapbooker.moving.offset.x;
					var y = e.pageY + scrapbooker.moving.offset.y;
					scrapbooker.moving.object.sourceObject.details.location.x = x;
					scrapbooker.moving.object.sourceObject.details.location.y = y;
					...
					$(document).unbind("mouseleave.scrapup");
					scrapbooker.updateDesktop(scrapbooker.moving.object);
					scrapbooker.moving.object.sourceObject.sendPosition();
					$(scrapbooker.moving.object).removeClass("dragging");
				});
		...
		scrapbooker.arrows();
		setTimeout(function() {scrapbooker.managePageTabs();},1000);
	},
	arrows : function() {
		if (!this.mobile) {
			for (var i = 0; i < this.items.length; i++) {
				...
				if (pos.left + $(this.items[i].object).outerWidth(true) < this.desktopWindow.element.scrollLeft)
					left = outside = true;
				if (pos.left > this.desktopWindow.element.scrollLeft + $(this.desktopWindow.element).width())
					right = outside = true;
				if (pos.top + $(this.items[i].object).outerHeight(true) < this.desktopWindow.element.scrollTop)
					top = outside = true;
				if (pos.top > this.desktopWindow.element.scrollTop + $(this.desktopWindow.element).height())
					bottom = outside = true;
				if (outside) {
					...
					if ((right || left) && !top && !bottom) {
						...
						if (center.y + $(this.items[i].arrow).outerHeight(true) > this.desktopWindow.height)
							bottom = true;
						if (center.y < 0)
							top = true;
						if (!top && !bottom)
							$(this.items[i].arrow).css("top", Math.round(center.y + 1) + "px");
					}
					if ((top || bottom) && !left && !right) {
						...
						if (center.x + $(this.items[i].arrow).outerWidth(true) > this.desktopWindow.width)
							right = true;
						if (center.x < 0)
							left = true;
						if (!left && !right)
							$(this.items[i].arrow).css("left", Math.round(center.x + 1) + "px");
					}
					if (bottom || top)
						$(this.items[i].arrow).css("top", "");
					if (left || right)
						$(this.items[i].arrow).css("left", "");
					if (left)
						$(this.items[i].arrow).addClass('arrowleft');
					if (right)
						$(this.items[i].arrow).addClass('arrowright');
					if (top)
						$(this.items[i].arrow).addClass('arrowtop');
					if (bottom)
						$(this.items[i].arrow).addClass('arrowbottom');
					if (!left && $(this.items[i].arrow).hasClass("arrowleft"))
						$(this.items[i].arrow).removeClass("arrowleft");
					if (!right && $(this.items[i].arrow).hasClass("arrowright"))
						$(this.items[i].arrow).removeClass("arrowright");
					if (!top && $(this.items[i].arrow).hasClass("arrowtop"))
						$(this.items[i].arrow).removeClass("arrowtop");
					if (!bottom && $(this.items[i].arrow).hasClass("arrowbottom"))
						$(this.items[i].arrow).removeClass("arrowbottom");
	...
	moveto : function(p) {
		...
		for (var i = 0; i < this.pages[this.whos][this.showingPage].items.length; i++) {
			if (this.pages[this.whos][this.showingPage].items[i] == this.contextItem.sourceObject)
				idx = i;
		}
		if (idx >= 0) {
	...
	deleteItem : function(o) {
		...
		for (var i = scrapbooker.pages.mine[scrapbooker.showingPage].items.length - 1; i >= 0; i--) {
			if (scrapbooker.pages.mine[scrapbooker.showingPage].items[i] == o.sourceObject) {
		...
		var p = {
			cmd : 'manageScrapbook',
			method : 'scrapbookDelete',
			context:scrapbooker,
			id : o.sourceObject.details.scrapbookid,
			callback : 'scrapbooker.gotResponse'
		};
		if (scrapbooker.pages.mine[scrapbooker.showingPage].details.type == "scrapbook") {
	...
	undeleteItem : function(o) {
		...
		var p = {
			cmd : 'manageScrapbook',
			method : 'scrapbookUndelete',
			context:scrapbooker,
			id : o.details.scrapbookid,
			callback : 'scrapbooker.gotResponse'
		};
		if (scrapbooker.pages.mine[scrapbooker.showingPage].details.type == "scrapbook") {
			p.fromPage = scrapbooker.pages.mine[scrapbooker.showingPage].details.id;
	...
	editItem : function(o) {
		...
		if ( typeof (o.sourceObject.details.note.html) == "string") {
			...
			$(this.editor).animate({
				width : (this.desktopWindow.width - (2 * margin)),
				height : (this.desktopWindow.height - (2 * margin)),
				opacity : 1,
				left : margin,
				top : margin
			}, 500, function() {
	...
	renderEditor : function() {
		...
		if (this.nicEditor == undefined) {
			this.nicEditor = new nicEditor({
				fullPanel : false,
				iconsPath : yoodoo.option.baseUrl + 'uploads/sitegeneric/image/dooits/nicEditorIcons.gif',
				buttonList : ['save', 'bold', 'italic', 'underline', 'left', 'center', 'right', 'justify', 'ol', 'ul', 'fontSize', 'fontFamily', 'fontFormat', 'indent', 'outdent', 'image', 'library', 'link', 'unlink', 'forecolor', 'bgcolor']
			});
		}
		this.nicEditor.panelInstance('scrapbookHTML', {
			hasPanel : true
		});
		this.htmlInstance = this.nicEditor.instanceById('scrapbookHTML');
		$(ins).find("button.cancel").bind("click", function() {
			$(scrapbooker.editor).fadeOut(500, function() {
				$(this).remove();
			});
		});
		$(ins).find("button.save").bind("click", function() {
			var txt = scrapbooker.htmlInstance.getContent();
			if (!(/<img/.test(txt)) && txt.replace(/<[^>]+>/g, '').replace(/( |&nbsp;)+/g, '') == '')
				txt = 'Empty';
		...
		$(this.htmlInstance.ne.nicPanel.panelElm.parentNode.parentNode).next().css({
			'max-height' : (h - 62)
		});
	},
	getScrapbookUser : function(user) {
		...
		if (ok)
			yoodoo.getScrapbookUser(this.viewingUser.details.id, this.pages.mine[this.showingPage].details.id, 'scrapbooker.gotUser');
	},
	gotUser : function(o) {
		o = yoodoo.decodeHTMLResponse(o);
		scrapbooker.parseElements(scrapbooker.parseXML(o));
		scrapbooker.showingUsers = false;
		scrapbooker.clearAndRedraw();
		scrapbooker.updateDesktop();
	},
	objects : {
		page : function(node) {
				...
				switch(node.childNodes[i].nodeName) {
						...
						for (var j = 0; j < node.childNodes[i].childNodes.length; j++) {
							...
							try {
								...
								if ( typeof (console) != "undefined" && typeof (console.log) != "undefined")
									console.log(item.type + " is not valid element");
							}
						}
					...
					case "users":
						...
						for (var j = 0; j < node.childNodes[i].childNodes.length; j++) {
							try {
								...
								if ( typeof (console) != "undefined" && typeof (console.log) != "undefined")
									console.log(node.childNodes[i].childNodes[j].nodeName + " is not valid element");
							}
						}
					...
					case "abuse":
						...
						for (var j = 0; j < node.childNodes[i].childNodes.length; j++) {
							...
							if (/^new Date/.test(abuse.reported))
								eval('abuse.reported=' + abuse.reported + ';');
			...
			if (this.details.autoPosition != undefined) {
				this.users.sort(function(a, b) {
					return a.details[a.page.details.autoPosition] - b.details[b.page.details.autoPosition];
				});
				if (this.details.autoPosition == "score")
					this.items.reverse();
		...
		event : function(node) {
			var o = new scrapbooker.itemObject(node, {
				className : "event",
				width : scrapbooker.desktopWindow.width - 66,
				pinColour : "staple",
				grabable : false,
				type : "event",
				movable : false
			...
			o.element = function() {
				this.params.classes = [];
				if (this.details.urgency != undefined && this.details.urgency != "")
					this.params.classes.push(this.details.urgency);
				if (this.details.complete != undefined && this.details.complete == "1")
					this.params.classes.push('complete');
				if (this.object !== null)
					return this.object;
				this.object = scrapbooker.eventContainer({
					classes : this.params.classes,
					className : this.params.className,
					width : this.params.width + "px",
					pinColour : this.params.pinColour,
		...
		episode : function(node) {
			var o = new scrapbooker.itemObject(node, {
				className : "episode",
				width : 170,
				pinColour : (scrapbooker.whos == "mine") ? "blue" : "staple",
				grabable : true,
				type : "episode",
				bookcaseType : 'book',
				movable : (scrapbooker.whos == "mine")
			});
			return o;
		},
		...
		file : function(node) {
			var o = new scrapbooker.itemObject(node, {
				className : "file",
				width : 170,
				pinColour : (scrapbooker.whos == "mine") ? "green" : "staple",
				grabable : true,
				type : "file",
				movable : (scrapbooker.whos == "mine")
			});
		...
		dooit : function(node) {
			var o = new scrapbooker.itemObject(node, {
				className : "dooit",
				width : 170,
				pinColour : (scrapbooker.whos == "mine") ? "purple" : "staple",
				grabable : true,
				type : "dooit",
				bookcaseType : 'dooit',
				movable : (scrapbooker.whos == "mine")
		...
		user : function(node) {
			...
			o.element = function() {
				if (this.details.classes == undefined)
					this.details.classes = [];
				this.me = (this.details.me != undefined && this.details.me);
				if (yoodoo.findindex("nocontext",this.details.classes) < 0)
					this.details.classes.push("nocontext");
				if (this.me && yoodoo.findindex("centeron",this.details.classes) < 0)
					this.details.classes.push("centeron");
				if (this.object !== null)
					return this.object;
				this.object = scrapbooker.itemContainer({
					className : "user",
					width : "170",
					pinColour : 'staple',
					grabable : false,
			...
			o.content = function() {
				...
				if (this.details.podium != undefined && this.details.podium) {
					...
					if (this.details.photo != undefined && this.details.photo != "")
						ins += "<img src='" + this.details.photo + "' />";
					if (this.details.score != undefined && this.details.score != "")
						ins += "<span class='score'>Score:<br /><span>" + this.details.score + "</span></span>";
					ins += '<div class="name">' + this.details.fullname + '</div>';
				} else {
					ins += '<div class="name">' + this.details.fullname + '</div>';
					if (this.details.score != undefined && this.details.score != "")
						ins += "<span class='score'>Score: " + this.details.score + "</span>";
					if (this.details.photo != undefined && this.details.photo != "")
						ins += "<img src='" + this.details.photo + "' />";
				}
				ins += '<div class="description">' + this.details.comment + '</div>';
				if (!this.details.me)
					ins += '<button type="button" class="userViewButton" onclick="scrapbooker.getScrapbookUser(this.parentNode.parentNode);">show their items</button>';
				return ins;
			};
			scrapbooker.users["user" + o.details.id] = o;
			if (o.details.me == "true")
				scrapbooker.me = o;
			o.owner = function() {
				var ins = '';
				if (this.details.photo != undefined && this.details.photo != "")
					ins += "<img width=20 style='vertical-align:middle' src='" + this.details.photo + "' />";
		...
		note : function(node) {
			var o = new scrapbooker.itemObject(node, {
				className : "note",
				width : 170,
				pinColour : (scrapbooker.whos == "mine") ? "red" : "staple",
				grabable : true,
				type : "note",
				movable : (scrapbooker.whos == "mine")
			});
			...
			o.content = function() {
				var ins = '';
				if (this.details.note.owner != undefined)
					ins += "<div class='owner'>" + this.details.note.owner + "</div>";
	...
	itemObject : function(node) {
		...
		if (this.params.bookcaseType != "none") {
			if (this.params.bookcaseType == "dooit")
				this.inBookcase = yoodoo.inBookcase("dooit", this.details.exercise.exerciseid);
			if (this.params.bookcaseType == "book")
				this.inBookcase = yoodoo.inBookcase("book", this.details.id);
			if (this.params.bookcaseType == "chapter") {
				this.inBookcase = yoodoo.inBookcase("book", this.details.episodeid);
			}
			if (this.params.bookcaseType == "keypoint")
				this.inBookcase = yoodoo.inBookcase("book", this.details.episodeid);
		}
		...
		this.element = function() {
			var shared = false;
			if (scrapbooker.whos != "mine")
				shared = this.isAttached();
			if (this.object !== null)
				return this.object;
			var remove = (scrapbooker.whos == "mine") || shared;
			if (scrapbooker.whos != "mine" && !shared && (this.params.type == "note" || this.inBookcase) && this.params.type != "none")
				this.grabable = true;
			this.object = scrapbooker.itemContainer({
				className : this.params.className,
				width : this.params.width + "px",
		...
		this.isAttached = function() {
			for (var i = 0; i < scrapbooker.pages.mine[scrapbooker.showingPage].items.length; i++) {
				if (this.details.type == scrapbooker.pages.mine[scrapbooker.showingPage].items[i].details.type && this.details.target_id == scrapbooker.pages.mine[scrapbooker.showingPage].items[i].details.target_id)
					return true;
			}
			return false;
		};
		...
		this.setGroups = function(groups) {
			...
			for (var id in groups) {
				if (this.inGroup(id) && !groups[id]) {
					toRemove.push(id);
					this.dropGroup(id);
					if (id == scrapbooker.pages.mine[scrapbooker.showingPage].details.id)
						dropFromPage = true;
				}
				if (!this.inGroup(id) && groups[id])
					toAdd.push(id);
			...
			//this.details.linked.groups=[];
			for (var i in toAdd)
			this.details.linked.groups.push(toAdd[i]);
			var params = {
				cmd : 'manageScrapbook',
			...
			scrapbooker.sendingPost();
			if (dropFromPage) {
				this.object.sourceObject.deleteItem();

				$(this.object).animate({
					top : (1 * this.details.location.y) + 20,
					opacity : 0
				}, function() {
					$(this).remove();
				});
		...
		this.deleteItem = function() {
			for (var i = scrapbooker.pages.mine[scrapbooker.showingPage].items.length - 1; i >= 0; i--) {
				if (scrapbooker.pages.mine[scrapbooker.showingPage].items[i] == this)
					scrapbooker.pages.mine[scrapbooker.showingPage].items.splice(i, 1);
			}
		};
		this.inGroup = function(id) {
			for (var a = 0; a < this.details.linked.groups.length; a++) {
				if (this.details.linked.groups[a] == id)
					return true;
			}
			return false;
		};
		this.dropGroup = function(id) {
			for (var a = this.details.linked.groups.length - 1; a >= 0; a--) {
				if (this.details.linked.groups[a] == id)
					this.details.linked.groups.splice(a, 1);
			}
		};
		this.content = function() {
			var ins = '<div class="name">' + this.details.name + '</div>';
			if (this.details.action != undefined && this.inBookcase)
				ins += "<button type='button' class='actionButton " + this.details.actionClass + "' onclick='" + this.details.action + "'>go</button>";
			if (this.details.description != undefined)
				ins += '<div class="description">' + this.details.description + '</div>';
			ins += this.comments();
			return ins;
		};
		...
		this.comments = function() {
			var ins = '';
			if (this.details.comments == undefined || typeof (this.details.comments) == "string")
			...
			if (scrapbooker.whos == "mine") {
				...
				if (scrapbooker.settings.add_comments) {
					...
					if (scrapbooker.settings.private_comments && (this.details.owner != undefined && this.details.owner != ""))
						ins += " - <a href='javascript:void(0)' onclick='scrapbooker.newComment(this,true)'>Add Privately</a>";
					ins += "</div>";
				}
				ins += "</div></div>";
			}
				...
				if (this.bar == null || this.bar == undefined) {
					...
					this.bar.bind("mousedown", function(e) {
						...
						$(yoodoo.container).bind("mousemove", function(e) {
							...
							if (y < 0)
								y = 0;
							if (y > scrapbooker.movingBar.maxst)
								y = scrapbooker.movingBar.maxst;
							$(scrapbooker.movingBar).css({
								top : y
							});
		...
		this.comment = function(com) {
			...
			var ins = '';
			if (arguments.length > 1)
				asElement = arguments[1];
			var when = com.createdat;
			if ( typeof (when) == "string")
				when = yoodoo.utcToLocal(inputs.stringToDate('Y-m-d H:i:s', when));
			var classes = ['comment'];
			var read = true;
			...
			if (com.datasnapshotid !== null && com.datasnapshotid != "") {
				click = "if(yoodoo.showDooit(" + this.details.exercise.exerciseid + "," + com.datasnapshotid + ")) scrapbooker.destroy();";
		...
		this.addComment = function(ip) {
			if (ip.value.replace(/ /g, '') != "") {
				...
				var params = {
					cmd : 'manageScrapbook',
					method : 'addComment',
					context:scrapbooker.newItemCache[cacheId],
					id : this.details.scrapbookid,
				...
				if (eventObject.type != "note")
					eventObject = yoodoo.bookcase.items[yoodoo.bookcaseIndex(eventObject.type, eventObject.target_id)];
				if (eventObject == undefined)
					eventObject = this.details;
				var reply = {
					content : ip.value
				};
		...
		this.commentAdded = function(reply) {
			try {
				...
				$(c).slideDown(1000, function() {
					scrapbooker.updateDesktop(this.sourceObject.object, true);
					this.sourceObject.commentSlider(true);
				});
		...
		this.bounce = function() {
			...
			if (this.details.location != undefined && this.details.location.y != undefined) {
				...
				$(o).animate({
					top : this.details.location.y - 20
				}, {
					duration : 500,
					ease : "sweep",
					complete : function() {
						$(this).animate({
							top : this.sourceObject.details.location.y + 20
						}, {
							duration : 500,
							ease : "sweep",
							complete : function() {
								$(this).animate({
									...
									complete : function() {
										$(this).animate({
											top : this.sourceObject.details.location.y
										}, {
											duration : 500,
											ease : "sweep"
										});
		...
		this.grabItem = function() {
			...
			if (this.details.note != undefined)
				params.noteid = this.details.note.noteid;
			if (this.details.exercise)
				params.exerciseid = this.details.exercise.exerciseid;
			yoodoo.sendPost(null, params);
		};
		this.bin = function() {
			this.object = null;
			...
			b.sourceObject = this;
			if (this.details.note != undefined)
				this.details.name = this.details.note.html.replace(/\<[^>]+\>/g, '').replace(/\</g, '&lt;').replace(/\>/g, '&gt;');
			...
			$(b).bind("click", function() {
				...
				this.sourceObject.page = scrapbooker.pages[scrapbooker.whos][scrapbooker.showingPage];
		...
		this.duplicate = function() {
			...
			o.params[k] = this.params[k];
			for (var k in this.details)
			o.details[k] = this.details[k];
			return o;
		};
		...
		this.deleteComment = function(com) {
			...
			$(item).slideUp(function() {
				var item = scrapbooker.parentOfClass(this, "scrapbookItem");
				$(this).remove();
				if ($(item).hasClass("scrapbookItem"))
					item.sourceObject.commentSlider();
			});
	...
	newLocation : function() {
		...
		return {
			x : x,
			y : y,
			z : this.pages[this.whos][this.showingPage].items.length
		};
	},
	toTop : function(item) {
	...
	scrollTo : function(element) {
		var toBottom = false;
		if (arguments.length > 1)
			toBottom = arguments[1];
		if (element.toBottom != undefined)
			toBottom = element.toBottom;
		...
		if (!this.mobile) {
			...
			if (y < 0)
				y = 0;
			$(this.desktopWindow.element).animate({
				'scrollTop' : y,
				'scrollLeft' : x
			}, {
				duration : 1000,
				easing : 'swing',
				step : function() {
					if (scrapbooker != undefined)
						scrapbooker.arrows();
				}
			});
		} else {
			var x = $(element).offset().left + Math.round($(element).width() / 2);
			var y = $(element).offset().top + Math.round($(element).height() / 2);
			//if (toBottom) y=$(element).position().top+Math.round($(element).height())-Math.round($(this.desktopWindow.element).height());
			//alert(x+","+y);
			x -= (yoodoo.option.width / 2);
			y -= (yoodoo.option.height / 2);
			y += 50;
			//this.mobileScrollTo(x,y);
			//alert(x+","+y);
			//$(window).animate({'pageXOffset':x,'pageYOffset':y});
			//window.scrollTo(x,y);
			//setTimeout('alert(window.pageXOffset);',5000);
		}
	},
	/*mobileScrollTo:function(x,y) {
	 this.lastScrollTo={x:window.pageXOffset,y:window.pageYOffset};
	 if (x<0) x=0;
	 if (y<0) y=0;
	 var maxX=$(this.desktop).width()-yoodoo.option.width;
	 var maxY=$(this.desktop).height()-yoodoo.option.height;
	 if (x>maxX) x=maxX;
	 if (y>maxY) y=maxY;
	 this.desiredScrollTo={x:x,y:y};
	 //$('body').animate({scrollTop:y,scrollLeft:x});
	 if (this.scrollTimer!=undefined && this.scrollTimer!=null) clearTimeout(this.scrollTimer);
	 this.scrollTimer=setTimeout('scrapbooker.processMobileScrollTo();',50);
	 },
	 processMobileScrollTo:function() {
	 this.pageX=window.pageXOffset;
	 this.pageY=window.pageYOffset;
	 var dx=(this.desiredScrollTo.x-this.pageX)/4;
	 var dy=(this.desiredScrollTo.y-this.pageY)/4;
	 this.pageX+=dx;
	 this.pageY+=dy;
	 window.scrollTo(this.pageX,this.pageY);
	...
	updateDesktop : function() {
		...
		if (this.autoScroll == null || !$(this.autoScroll).hasClass("centeron")) {
			...
			if (unread.length > 0) {
				...
				if (!$(this.autoScroll).hasClass("scrapbookItem"))
					this.autoScroll = null;
			}
		}
		var toBottom = false;
		...
		for (var i = 0; i < this.items.length; i++) {
			...
			if (maxx < r + this.boundary)
				maxx = r + this.boundary;
			if (maxy < b + this.boundary)
				maxy = b + this.boundary;
			if (minx > l - this.boundary)
				minx = l - this.boundary;
			if (miny > t - this.boundary)
				miny = t - this.boundary;
		}
		if (minx < 0 || miny < 0) {
			maxx -= minx;
			maxy -= miny;
			...
			if (($(this.desktop).width() > maxx || $(this.desktop).height() > maxy) && (minx >= 0 && miny >= 0)) {
				$(this.desktop).animate({
					...
					complete : function() {
						if (scrapbooker === undefined)
							return false;
						if (scrapbooker.autoScroll !== null)
							scrapbooker.scrollTo(scrapbooker.autoScroll);
					}
				});
			} else {
				...
				if (this.autoScroll !== null)
					this.scrollTo(this.autoScroll);
			}
		} else {
			$(this.desktop).css("width", maxx + "px");
			$(this.desktop).css("height", maxy + "px");
			if (minx < 0 || miny < 0) {
				window.scrollLeft -= minx;
				window.scrollTop -= miny;
			}
			if (this.autoScroll !== null)
				this.scrollTo(this.autoScroll);
	...
	eventContainer : function(params) {
		...
		if (params.source != undefined)
			d.sourceObject = params.source;

		var z = parseInt(this.zIndex, 10);
		if (params.source.details.location != undefined && params.source.details.location.z != undefined)
			z += parseInt(params.source.details.location.z, 10);
		$(d).css("z-index", z);
		var x = 0;
		if (params.source.details.location != undefined && params.source.details.location.x != undefined)
			x += parseInt(params.source.details.location.x, 10);
		$(d).css("left", x + 'px');
		var y = 0;
		if (params.source.details.location != undefined && params.source.details.location.y != undefined)
			y += parseInt(params.source.details.location.y, 10);
		$(d).css("top", y + 'px');

		$(d).addClass("scrapbookItem");
		if (params.classes != undefined) {
			for (var p = 0; p < params.classes.length; p++)
				$(d).addClass(params.classes[p]);
		}
		if (params.className != undefined)
			$(d).addClass(params.className);
		if (params.width != undefined)
			$(d).css("width", params.width);
		if (params.height != undefined)
			$(d).css("height", params.height);
		var content = 'empty';
		if (params.content != undefined)
			content = params.content;
		ins = '<div class="scrapbookItemContent">';
		if (params.source.details.heading != undefined)
			ins += "<div class='heading'>" + params.source.details.heading + "</div>";
	...
	itemContainer : function(params) {
		...
		if (params.height != undefined)
			$(d).css("height", params.height);
		var pinColour = 'red';
		if (params.pinColour != undefined)
			pinColour = params.pinColour;
		var content = 'empty';
		if (params.content != undefined)
			content = params.content;
		...
		if (scrapbooker.whos == "mine" && !scrapbooker.showingUsers && scrapbooker.hasGroups)
			ins += '<button type="button" class="public' + ((groups.length > 0) ? ' on' : '') + '" title="' + ((groups.length > 0) ? this.publicText.on : this.publicText.off) + '"></button>';

		if ((params.remove == undefined || params.remove)) {
			if (scrapbooker.whos == "mine") {
				ins += '<div class="tools">';
				if (params.source.editable)
					ins += '<button type="button" class="edit" title="edit"></button>';
			...
			if (scrapbooker.whos != "mine") {
				ins += '<div class="tools">';
				ins += '<span style="font-size:10px;line-height:11px">Not in your<br />bookcase</span>';
				ins += "</div>";
			}
		}
		if (params.source.details.created_at != undefined && params.source.details.created_at != "") {
			...
			if ( typeof (dt) == "string")
				dt = yoodoo.utcToLocal(inputs.stringToDate('Y-m-d H:i:s', dt));
		}
		ins += content + '</div>';
		$(d).html(ins);
		return d;
		...
		if (node.nodeName == "comments") {
			...
			for (var i = 0; i < node.childNodes.length; i++) {
				if (node.childNodes[i].childNodes.length == 1 && (node.childNodes[i].childNodes[0].nodeName == '#text' || node.childNodes[i].childNodes[0].nodeName == '#cdata-section')) {
					...
					if (/^\d{2,4}-\d{1,2}-\d{1,2} \d{1,2}:\d{1,2}:\d{1,2}$/.test(params[node.childNodes[i].nodeName]))
						params[node.childNodes[i].nodeName] = yoodoo.utcToLocal(this.parseDate(params[node.childNodes[i].nodeName]));
					if (params[node.childNodes[i].nodeName] == "true")
						params[node.childNodes[i].nodeName] = true;
					if (params[node.childNodes[i].nodeName] == "false")
						params[node.childNodes[i].nodeName] = false;
					if (params[node.childNodes[i].nodeName] == undefined)
						params[node.childNodes[i].nodeName] = '';
	...
	autoPosition : {
		score : function(items) {
			...
			if (items.length >= 2) {
				...
				items[1].details.location = {
					x : -scrapbooker.autoLayout.spread.x,
					y : scrapbooker.autoLayout.spread.y / 3,
					z : z--
				};
			...
			if (items.length >= 3) {
				...
				items[2].details.location = {
					x : scrapbooker.autoLayout.spread.x,
					y : scrapbooker.autoLayout.spread.y / 2,
					z : z--
				};
	...
	abridgeDate : function(d) {
		...
		if (today.getFullYear() != d.getFullYear())
			return inputs.formatDate('jS M \'y', d);
		if (today.getMonth() != d.getMonth())
			return inputs.formatDate('jS M', d);
		if (today.getDate() != d.getDate())
			return inputs.formatDate('g:ia jS', d);
	...
	uploader : function() {
		...
		if ( typeof (yoodoo) != "undefined" && yoodoo.sitehash != undefined)
			sitehash = yoodoo.sitehash;
		var userhash = 'missingUser';
		if ( typeof (yoodoo) != "undefined" && yoodoo.loginCode != undefined)
			userhash = yoodoo.loginCode;
		...
		dialog.show({
			id : 'scrapbookuploader',
			title : 'Upload',
			content : ins,
			withBlackoutClickClose : false
		});
	},
	...
	imageSelected : function(img) {
		...
		for (var i = 0; i < imgs.length; i++) {
			if (imgs[i].src == img.source)
				imgs[i].title = img.title;
		}
		dialog.hide();
	},
	fetchScrapbookPage : function() {
		...
		if (arguments.length > 0)
			forcePage = arguments[0];
		var callback = 'scrapbooker.gotScrapbookPage';
		if (arguments.length > 1)
			callback = arguments[1];
		var params = {context:scrapbooker};
		/*if(this.recycle.length>0) {
		 params.removeItems=[];
