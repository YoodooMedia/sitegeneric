...
this.mobile=false;
	...
	dispose:function() {
		$(yoodoo.widget).css({width:yoodoo.option.width,height:yoodoo.option.height});
	...
	setXML:function(xml) {
		...
		if(!yoodoo.isApp || true) this.managePageTabs();
	...
	destroy:function() {
		if (this.mobile) {
			$(yoodoo.container).css({display:'none'});
	...
	createDesktop:function() {
		if (yoodoo.isApp && false) {
			...
			$(this.scrapbookPages).addClass("scrapbookToolbar");// "+yoodoo.class_prefix+"_overlay");
			//if (yoodooPlaya!=undefined) {
			$(this.scrapbookPages).addClass(yoodoo.class_prefix+"_toolbar").css({position:'fixed',left:0,right:0,top:-1,'padding-top':1,'z-index':999,width:"100%"});
			//}
			...
			$(yoodoo.widget).css({width:'auto',height:'auto'});
			$(yoodoo.container).css({display:'none'});
	...
	drawToolbar:function() {
		...
		for(var p=0;p<this.pages[this.whos].length;p++) {
			...
			opt.value=p;
			if (p==this.showingPage) opt.selected=true;
			sel.appendChild(opt);
		}
	...
	createItems:function() {
		...
		$(elements).find('button.public').bind("click",function(e){
			...
			if (scrapbooker.mobile) {
				$(source.blockout).css({width:scrapbooker.desktopWindow.width,height:scrapbooker.desktopWindow.height});
			}
		...
		$(elements).bind("click",function(e){
			if ((e.target==this || e.target.parentNode==this || $(e.target).hasClass("description")) && $(e.target).data('events')==undefined) {
				...
				if (!yoodoo.isApp || true) scrapbooker.scrollTo(this);
			}
		});
...
//alert(e.originalEvent.touches[0].pageX);
	...
	deleteItem:function(o) {
			...
			scrapbooker.sendingPost();
			var p={cmd:'manageScrapbook',method:'scrapbookDelete',id:o.sourceObject.details.scrapbookid,callback:'scrapbooker.gotResponse'};
			if(scrapbooker.pages.mine[scrapbooker.showingPage].details.type=="scrapbook") {
...
//console.log($(this.object).find(".comment"));
		...
		this.setGroups=function(groups) {
			...
			if(dropFromPage) {
				...
				$(this.object).animate({top:(1*this.details.location.y)+20,opacity:0},function() {$(this).remove();});
				scrapbooker.updateDesktop();
			}
		};
...
//alert(n.css("display"));
		...
		//}
			...
			if (!this.mobile) {
				if (($(this.desktop).width()>maxx || $(this.desktop).height()>maxy) && (minx>=0 && miny>=0)){
					$(this.desktop).animate({width:maxx+"px",height:maxy+"px"},{duration:500,easing:'swing',
						step:function(){
							if (scrapbooker===undefined) return false;scrapbooker.arrows();
						},
						complete:function(){
							if (scrapbooker===undefined) return false;
							if (scrapbooker.autoScroll!==null) scrapbooker.scrollTo(scrapbooker.autoScroll);
						}
					});
				}else{
	...
	itemContainer:function(params) {
		...
		if (scrapbooker.whos=="mine" && !scrapbooker.showingUsers && scrapbooker.hasGroups) ins+='<button type="button" class="public'+((groups.length>0)?' on':'')+'" title="'+((groups.length>0)?this.publicText.on:this.publicText.off)+'"></button>';
