...
scrapbook={
	...
	createItems:function() {
		...
		if (userButton) {
			...
			var who='';
			if (scrapbook.whos!="mine") who='&nbsp;['+scrapbook.users[scrapbook.whos].details.fullname+']';
		...
		for(i=0;i<items.length;i++) {
			if(this.pages[this.whos][this.showingPage].details.autoPosition==undefined && (items[i].details.location==undefined || isNaN(items[i].details.location.x) || isNaN(items[i].details.location.y) || isNaN(items[i].details.location.z))) items[i].details.location=scrapbook.newLocation();
		}
	...
	objects:{
		page:function(node) {
			...
			if (this.users.length>0) {
				this.users.sort(function(a,b) {return a.details.score-b.details.score;});
				this.users.reverse();
		...
		episode:function(node) {
			var o=new scrapbook.itemObject(node,{className:"episode",width:150,pinColour:(scrapbook.whos=="mine")?"blue":"staple",grabable:true,type:"episode",bookcaseType:'book'});
			return o;
		},
		...
		keypoint:function(node) {
			var o=new scrapbook.itemObject(node,{className:"keypoint",width:150,pinColour:(scrapbook.whos=="mine")?"blue":"staple",grabable:true,type:"keypoint",bookcaseType:'keypoint'});
			return o;
		},
		file:function(node) {
			var o=new scrapbook.itemObject(node,{className:"file",width:150,pinColour:(scrapbook.whos=="mine")?"green":"staple",grabable:true,type:"file"});
			return o;
		},
		dooit:function(node) {
			var o=new scrapbook.itemObject(node,{className:"dooit",width:150,pinColour:(scrapbook.whos=="mine")?"purple":"staple",grabable:true,type:"dooit",bookcaseType:'dooit'});
			return o;
		},
		chapter:function(node) {
			var o=new scrapbook.itemObject(node,{className:"chapter",width:150,pinColour:(scrapbook.whos=="mine")?"blue":"staple",grabable:true,type:"chapter",bookcaseType:'chapter'});
			return o;
		},
		user:function(node) {
			var o=new scrapbook.itemObject(node,{className:"user",width:150,pinColour:"staple",type:"user",movable:false});
			o.element=function() {
				...
				if (this.object!==null) return this.object;
				this.object=scrapbook.itemContainer({className:"user",width:"150",pinColour:'staple',grabable:false,content:this.content(),source:this,classes:this.details.classes,movable:false,privacy:false,remove:false});
				return this.object;
			};
			o.content=function() {
				var ins='';
		...
		note:function(node) {
			var o=new scrapbook.itemObject(node,{className:"note",width:150,pinColour:(scrapbook.whos=="mine")?"red":"staple",grabable:true,type:"note"});
			if (node===null) {
	...
	itemObject:function(node){
		...
		this.setGroups=function(groups) {
			...
			this.details.linked.groups=[];
			for(var id in groups) this.details.linked.groups.push(id);
		...
		this.comment=function(com){
			...
			if (com.owner!=undefined) ins+="<div class='from'>"+com.owner.name+"</div>";
			ins+=com.content;
			if(asElement) {
	...
	itemContainer:function(params) {
		...
		if (scrapbook.whos=="mine" && !scrapbook.showingUsers) ins+='<button type="button" class="public'+((groups.length>0)?' on':'')+'" title="'+((groups.length>0)?this.public.on:this.public.off)+'"></button>';
		...
		if ((params.remove==undefined || params.remove)) {
			...
			ins+='<div class="tools">';
			ins+='<button type="button" class="grab" title="Share this too">share</button>';
			ins+="</div>";
		}
	...
	fetchScrapbookPage:function() {
		...
		yoodoo.fetchScrapbookPage(this.showingPage,'scrapbook.gotScrapbookPage',params);
	},
