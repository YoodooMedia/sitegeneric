...
yoodoo.welcome = function() {
	...
	$(yoodoo.welcomeContainers.tabcontent[2]).append(yoodoo.welcomeContainers.topPanel).append(yoodoo.welcomeContainers.bottomPanel);
	...
	for (var i = 0; i < ips.length; i++) {
		...
		$(row).append($(input).attr("type", "text").attr("maxlength", ips[i].length).val(v));
		$(details).append(row);
	}
...
yoodoo.processFilterBookcase = function() {
	...
	for (var i = 0; i < this.bookcase.items.length; i++) {
		...
		if (this.bookcase.items[i].hideAfterTilComplete && !this.bookcase.items[i].completed && this.intervention === null)
			hideRest = true;
	...
	widgets : {
		progress : function(src) {
			...
			this.build = function() {
				...
				var as = '';
				var score = '<span style="float:right;color:#AAA;font-size:11px" title="Your Yoodoo Score">Score:' + yoodoo.user.score + '</span>';
				if (this.averageScore > 0)
					as = 'Average quiz score: <b>' + this.averageScore.toFixed(1) + '%</b>';
				$(f).html(score + '<b>' + yoodoo.home_progress_text + '</b><div class="progressBlock"><div style="width:' + yoodoo.home_percentage_complete + '%"></div><span>' + yoodoo.home_percentage_complete + '%</span></div><div>' + yoodoo.home_level_text + '</div><div id="averageScoreDiv">' + as + '</div><div class="episodeText">' + yoodoo.home_episode_text + '</div>');
...
yoodoo.bubble = function(obj,text) {
	...
	$(obj).unbind('mouseenter').bind("mouseenter",function(e) {
		...
		var pos=$(this).offset();
		$(this.bubble).css({
			left:pos.left+($(this).width()/2)-($(this.bubble).width()/2),
			top:pos.top-$(this.bubble).height()-20
