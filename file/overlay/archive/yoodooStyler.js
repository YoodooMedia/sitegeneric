var yoodooStyler={
	...
	shadow:function(obj,colour,x,y,distance) {
		...
		var styles={};
		styles.o={'-o-box-shadow':x+'px '+y+'px '+distance+'px '+this.rgbToHex(colour)};
		styles.moz={'-moz-box-shadow':x+'px '+y+'px '+distance+'px '+this.rgbToHex(colour)};
		styles.khtml={'-khtml-box-shadow':x+'px '+y+'px '+distance+'px '+this.rgbToHex(colour)};
		styles.webkit={'-webkit-box-shadow':x+'px '+y+'px '+distance+'px '+this.rgbToHex(colour)};
		styles.standard={'box-shadow':x+'px '+y+'px '+distance+'px '+this.rgbToHex(colour)};
		var definitions=[];
		for(var s in styles) {
	...
	gradient:function(obj,colour,lighten,brighten,horizontal,reversed,createStylesheet) {
		...
		var fromColour={r:colour.r,g:colour.g,b:colour.b};
		...
		var toColour=this.tint({r:fromColour.r,g:fromColour.g,b:fromColour.b},colour.tint.lightness,colour.tint.brightness);
		//toColour=this.tint(toColour,lighten,brighten);
		var colours={};
		if (reversed) {
			colours={from:this.rgbToHex(toColour),to:this.rgbToHex(fromColour)};
		}else{
			colours={from:this.rgbToHex(fromColour),to:this.rgbToHex(toColour)};
		}
		var styles={};
		styles.old={background:colours.from};
		styles.ff={background:'-moz-linear-gradient('+(horizontal?'left':'top')+', '+colours.from+' 0%, '+colours.to+' 100%)'};
		styles.chromeOld={background:'-webkit-gradient(linear, '+(horizontal?'left top, right top,':'left top, left bottom,')+' color-stop(0%,'+colours.from+'), color-stop(100%,'+colours.to+'))'};
		styles.chrome={background:'-webkit-linear-gradient('+(horizontal?'left':'top')+', '+colours.from+', 0%, '+colours.to+',100%)'};
		styles.opera={background:'-o-linear-gradient('+(horizontal?'left':'top')+', '+colours.from+' 0%, '+colours.to+'100%)'};
		styles.ms={background:'-ms-linear-gradient('+(horizontal?'left':'top')+', '+colours.from+' 0%, '+colours.to+' 100%)'};
		styles.w3c={background:'linear-gradient('+(horizontal?'left':'top')+', '+colours.from+' 0%, '+colours.to+' 100%)'};
		styles.ie={filter:"progid:DXImageTransform.Microsoft.gradient( startColorstr='"+colours.from+"', endColorstr='"+colours.to+"',GradientType="+(horizontal?'1':'0')+" )"};
		var definitions=[];
		for(var s in styles) {
			for(var ss in styles[s]) {
				if(createStylesheet && typeof(obj)=="string") {
	...
	borderRadius:function(obj,r) {
		...
		styles.o={'-o-border-radius':rads};
		styles.moz={'-moz-border-radius':rads};
		styles.khtml={'-khtml-border-radius':rads};
		styles.webkit={'-webkit-border-radius':rads};
		styles.standard={'border-radius':rads};
		var definitions=[];
		for(var s in styles) {
			for(var ss in styles[s]) {
				if(createStylesheet && typeof(obj)=="string") {
	...
	rgbToHSV:function(col) {
		...
		var delta=maxV-minV;
		var hsv={h:0,s:Math.round(100*delta/maxV),v:Math.round(100*(maxV/255))};
		if (delta==0) {
			hsv.h=0;
		}else if (col.r>=maxV) {
	...
	hsvToRGB:function(hsv) {
		...
		switch(sect) {
			...
			break;
			case 1:
				rgb={r:255-rem,g:255,b:0};
			break;
			case 2:
				rgb={r:0,g:255,b:rem};
			break;
			case 3:
				rgb={r:0,g:255-rem,b:255};
			break;
			case 4:
				rgb={r:rem,g:0,b:255};
			break;
			case 5:
				rgb={r:255,g:0,b:255-rem};
			break;
		}
		rgb.r*=hsv.v/100;
		rgb.g*=hsv.v/100;
	...
	build_styles:function() {
		if (yoodoo.styles.dooittext===undefined) yoodoo.styles.dooittext={r:50,g:50,b:50};
		var newcss={};
		if (yoodoo.isApp) {
			newcss['.overlayFooter']={position:'fixed',left:0,right:0,width:"100%"};
		}
		...
		if (yoodoo.styles.backgroundImage!==undefined && yoodoo.styles.backgroundImage!==null && yoodoo.styles.backgroundImage!="") {
			...
			newcss['#yoodooWidget, #yoodooContainerContent']={'background-image':'url('+yoodoo.styles.backgroundImage+') !important','background-size':'cover'};
		}else{
		...
		newcss['.'+yoodoo.class_prefix+'_overlay, #yoodooWidget .welcome .tabs button.on']={'background-color':yoodooStyler.rgbToCSS(yoodoo.styles.overlay.background)};
		newcss['#yoodooWait']={'background-color':yoodooStyler.rgbToCSS(yoodoo.styles.base)};
		//yoodooStyler[yoodoo.styles.base.gloss?'gradientGloss':'gradient']('#yoodooWait',yoodoo.styles.base,0,0,false,true);
		if (!yoodoo.isApp) yoodooStyler.borderRadius('#yoodooContainerContent, #yoodooWidget, #yoodooWhiteout, #yoodooWait, .'+yoodoo.class_prefix+'_episode_container, .'+yoodoo.class_prefix+'_bookcase_container, .'+yoodoo.class_prefix+'_keypointDisplay',yoodoo.styles.boundary.radius);
		...
		newcss['#yoodooWidget .'+yoodoo.class_prefix+'_keypointDisplay p']={color:yoodooStyler.rgbToHex(yoodoo.styles.keypoint.text)};
	...
	newcss['.'+yoodoo.class_prefix+'_toolbar,#yoodooWidget .overlayFooter,#yoodooWidget .overlayFooter .footerWarning']={'background':yoodooStyler.rgbToHex(yoodoo.styles.toolbar.background),'color':yoodooStyler.rgbToHex(yoodoo.styles.toolbar.text)};
	newcss['.'+yoodoo.class_prefix+'_toolbar, .'+yoodoo.class_prefix+'_toolbar a,#yoodooWidget .overlayFooter button.footerbutton']={'color':yoodooStyler.rgbToHex(yoodoo.styles.toolbar.button.text),'background':yoodooStyler.rgbToHex(yoodoo.styles.toolbar.button.background)};
	newcss['.'+yoodoo.class_prefix+'_toolbar a:hover, #yoodooWidget.isMouse .overlayFooter button.footerbutton:hover']={'color':yoodooStyler.rgbToHex(yoodoo.styles.toolbar.button.textHover),'background':yoodooStyler.rgbToHex(yoodoo.styles.toolbar.button.hover)};
	newcss['.'+yoodoo.class_prefix+'_toolbar button,#yoodooWidget .overlayFooter button.done']={'color':yoodooStyler.rgbToHex(yoodoo.styles.toolbar.done.text),'background':yoodooStyler.rgbToHex(yoodoo.styles.toolbar.done.background)};
	newcss['.'+yoodoo.class_prefix+'_toolbar button,#yoodooWidget.isMouse .overlayFooter button.done:hover']={'color':yoodooStyler.rgbToHex(yoodoo.styles.toolbar.done.textHover),'background':yoodooStyler.rgbToHex(yoodoo.styles.toolbar.done.backgroundHover)};
	newcss['.'+yoodoo.class_prefix+'_toolbar button,#yoodooWidget .overlayFooter>*>button.on']={'color':yoodooStyler.rgbToHex(yoodoo.styles.toolbar.button.textOn),'background':yoodooStyler.rgbToHex(yoodoo.styles.toolbar.button.on)};
	newcss['#yoodooWidget .overlayFooter .dialog']={'border-top':'1px solid '+yoodooStyler.rgbToHex(yoodoo.styles.toolbar.background),'color':yoodooStyler.rgbToHex(yoodoo.styles.toolbar.dialog.text),'background':yoodooStyler.rgbToHex(yoodoo.styles.toolbar.dialog.background)};
	newcss['#yoodooWidget .overlayFooter .dialog>div>button']={'color':yoodooStyler.rgbToHex(yoodoo.styles.toolbar.dialog.buttontext),'background':yoodooStyler.rgbToHex(yoodoo.styles.toolbar.dialog.buttonbackground)};
	newcss['#yoodooWidget .overlayFooter .dialog>div>button:hover']={'color':yoodooStyler.rgbToHex(yoodoo.styles.toolbar.dialog.buttontextHover),'background':yoodooStyler.rgbToHex(yoodoo.styles.toolbar.dialog.buttonbackgroundHover)};
	...
	/*if (BrowserDetect.browser!="Explorer" || (BrowserDetect.browser=="Explorer" && BrowserDetect.version>9)) {
		...
		newcss['#yoodooWidget .widgetContainer>div']={color:yoodooStyler.rgbToCSS(yoodoo.styles.widget.text)};

		if (yoodoo.styles.button.colour.gloss===false && yoodoo.styles.button.colour.tint.lightness==0 && yoodoo.styles.button.colour.tint.brightness==0) {
			newcss['.'+yoodoo.class_prefix+'_buttons button']={background:yoodooStyler.rgbToHex(yoodoo.styles.button.colour)};
		}else{
		...
		if (yoodoo.styles.button.colourHover.gloss===false && yoodoo.styles.button.colourHover.tint.lightness==0 && yoodoo.styles.button.colourHover.tint.brightness==0) {
			newcss['.'+yoodoo.class_prefix+'_buttons button:hover']={background:yoodooStyler.rgbToHex(yoodoo.styles.button.colourHover)};
		}else{
		...
		if (yoodoo.styles.button.colourOn.gloss===false && yoodoo.styles.button.colourOn.tint.lightness==0 && yoodoo.styles.button.colourOn.tint.brightness==0) {
			newcss['.'+yoodoo.class_prefix+'_buttons .'+yoodoo.class_prefix+'_bookcaseButtons button']={background:yoodooStyler.rgbToHex(yoodoo.styles.button.colourOn)};
		}else{
		...
		if (yoodoo.styles.button.colourOff.gloss===false && yoodoo.styles.button.colourOff.tint.lightness==0 && yoodoo.styles.button.colourOff.tint.brightness==0) {
			newcss['.'+yoodoo.class_prefix+'_buttons .'+yoodoo.class_prefix+'_bookcaseButtons button.off']={background:yoodooStyler.rgbToHex(yoodoo.styles.button.colourOff)};
		}else{
		...
		if (!yoodoo.is_touch) {
			if (yoodoo.styles.button.colourOnHover.gloss===false && yoodoo.styles.button.colourOnHover.tint.lightness==0 && yoodoo.styles.button.colourOnHover.tint.brightness==0) {
				newcss['.'+yoodoo.class_prefix+'_buttons .'+yoodoo.class_prefix+'_bookcaseButtons button:hover']={background:yoodooStyler.rgbToHex(yoodoo.styles.button.colourOnHover)};
			}else{
			...
			if (yoodoo.styles.button.colourOffHover.gloss===false && yoodoo.styles.button.colourOffHover.tint.lightness==0 && yoodoo.styles.button.colourOffHover.tint.brightness==0) {
				newcss['.'+yoodoo.class_prefix+'_buttons .'+yoodoo.class_prefix+'_bookcaseButtons button.off:hover']={background:yoodooStyler.rgbToHex(yoodoo.styles.button.colourOffHover)};
			}else{
		...
		newcss['#yoodooWidget .tip .scrollButton']={border:'1px solid '+yoodooStyler.rgbToHex(yoodoo.styles.highlight)};
		...
		newcss['#yoodooWidget .tip .warningNotice>div']={color:yoodooStyler.rgbToHex(yoodooStyler.tint(yoodoo.styles.warning,0.2,1.5)),border:'1px solid '+yoodooStyler.rgbToHex(yoodooStyler.tint(yoodoo.styles.warning,0.0,2))};
		if (typeof(yoodooPlaya)!='undefined') {
			newcss['.yd_with_levels .yd_thumb']={height:Math.floor(yoodooPlaya.bookcase.slider.height/2)+"px"};
			newcss['.yd_without_levels .yd_thumb']={height:yoodooPlaya.bookcase.slider.height+"px"};
			newcss['.yd_level']={height:Math.ceil(yoodooPlaya.bookcase.slider.height/2)+"px",'line-height':Math.ceil(yoodooPlaya.bookcase.slider.height/2)+"px",'font-size':Math.ceil(yoodooPlaya.bookcase.slider.height/3)+"px"};
			newcss['.'+yoodoo.class_prefix+'_book_title']={'font-size':Math.round(yoodooPlaya.book.spineTextHeight*yoodooPlaya.book.spine)+'px'};
		}
		if (!yoodoo.is_touch) newcss['.yd_thumb:hover div']={background:"#fff"};
		newcss['#yoodooWidget h2']={color:yoodooStyler.rgbToHex(yoodoo.styles.h2)};
		newcss['#yoodooWidget h3']={color:yoodooStyler.rgbToHex(yoodoo.styles.h3)};
		newcss['#yoodooWidget button.tab']={color:yoodooStyler.rgbToHex(yoodoo.styles.text)};
		
		newcss['#yoodooContainerContent h3,#yoodooContainerContent .login,#yoodooContainerContent .welcome']={color:yoodooStyler.rgbToHex(yoodoo.styles.text)};
		newcss['#yoodooContainerContent h3 a,#yoodooContainerContent .login a,#yoodooContainerContent .welcome a']={color:yoodooStyler.rgbToHex(yoodoo.styles.link)};
		newcss['#yoodooContainerContent h3 a:hover,#yoodooContainerContent .login a:hover,#yoodooContainerContent .welcome a:hover']={color:yoodooStyler.rgbToHex(yoodoo.styles.linkHover)};
		newcss['#yoodooContainerContent h3 a,#yoodooContainerContent .login a,#yoodooContainerContent .welcome a']={color:yoodooStyler.rgbToHex(yoodoo.styles.link)};
		newcss['#yoodooContainerContent h3 a:hover,#yoodooContainerContent .login a:hover,#yoodooContainerContent .welcome a:hover']={color:yoodooStyler.rgbToHex(yoodoo.styles.linkHover)};
		newcss['#yoodooContainerContent .'+yoodoo.class_prefix+'_overlay, #yoodooWidget .welcome .tabs button.on']={color:yoodooStyler.rgbToCSS(yoodoo.styles.overlay.text)};
		newcss['#yoodooContainerContent .'+yoodoo.class_prefix+'_overlay a']={color:yoodooStyler.rgbToHex(yoodoo.styles.overlay.link)};
		newcss['#yoodooContainerContent .'+yoodoo.class_prefix+'_overlay a:hover']={color:yoodooStyler.rgbToHex(yoodoo.styles.overlay.linkHover)};

		var baseDup={r:yoodoo.styles.base.r,g:yoodoo.styles.base.g,b:yoodoo.styles.base.b};
		newcss['.yd_keypointHeader button']={background:yoodooStyler.rgbToHex(baseDup),color:yoodooStyler.rgbToHex(yoodoo.styles.text)};
		baseDup={r:yoodoo.styles.base.r,g:yoodoo.styles.base.g,b:yoodoo.styles.base.b};
		newcss['.yd_keypointFooter button']={background:yoodooStyler.rgbToHex(yoodooStyler.tint(baseDup,yoodoo.styles.base.tint.lightness,yoodoo.styles.base.tint.brightness)),color:yoodooStyler.rgbToHex(yoodoo.styles.text)};
		newcss['#exercise_content, #exercise_content p']={color:yoodooStyler.rgbToHex(yoodoo.styles.dooittext)};

		if (yoodoo.gradientFill) {
			var col=yoodooStyler.rgbToHex(yoodooStyler.tint(yoodoo.styles.book.colour,yoodoo.styles.book.border.lighten,yoodoo.styles.book.border.brighten));
			newcss['.'+yoodoo.class_prefix+"_bookcaseItem"]={'border':yoodoo.styles.book.border.width+'px solid '+col};
			...
			var col=yoodooStyler.rgbToHex(yoodooStyler.tint(yoodoo.styles.book.colour,yoodoo.styles.book.border.lighten,yoodoo.styles.book.border.brighten));
			newcss['.'+yoodoo.class_prefix+"_bookcaseItem"]={'border':yoodoo.styles.book.border.width+'px solid '+col,'background':yoodooStyler.rgbToHex(yoodoo.styles.book.colour)};
		}
		...
		newcss['.'+yoodoo.class_prefix+'_keypoint']={color:yoodooStyler.rgbToHex(yoodoo.styles.keypoints.text)};
		...
		newcss['.'+yoodoo.class_prefix+'_keypoint.'+yoodoo.class_prefix+'_past_keypoint']={color:yoodooStyler.rgbToHex(yoodoo.styles.keypoints.past.text)};
		...
		newcss['.'+yoodoo.class_prefix+'_keypoint.'+yoodoo.class_prefix+'_current_keypoint']={color:yoodooStyler.rgbToHex(yoodoo.styles.keypoints.current.text)};
		...
		newcss['.'+yoodoo.class_prefix+'_complete,.'+yoodoo.class_prefix+'_complete button']={color:yoodooStyler.rgbToHex(yoodoo.styles.episodeOverlay.text)};
