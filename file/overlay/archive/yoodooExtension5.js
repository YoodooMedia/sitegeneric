...
yoodoo.dooit_standardize=function(obj) {
		var standard={
			...
			init: function() {
				...
				if (this.key !== null && this.containers.container !== undefined && this.containers.container !== null) {
			...
			loadFields: function() {
				...
				if (yoodoo.dooit!==undefined) {
					...
					for(var k in yoodoo.dooit.params.fields) {
						if (k!=this.key && k!=this.globalkey) this.fields[k]=yoodoo.dooit.params.fields[k][1];
					}
				}else{
					...
					if (this.key !== null) try{eval('ui_questions.value='+array_of_fields[this.key][1]+';');}catch(e){ui_questions.value=array_of_fields[this.key][1];};
					if (this.globalkey !== null)  try{eval('ui_questions.schema='+array_of_fields[this.globalkey][1]+';');}catch(e){};
					for(var k in array_of_fields) {
						if (k!=this.key && k!=this.globalkey) try{eval('ui_questions.fields[k]='+array_of_fields[k][1]+';');}catch(e){};
					}
				}
			},
			finishable: function() {
...
yoodoo.login = function(un, pw) {
	this.working(true, yoodoo.w('attemptinglogin'));
...
yoodoo.parentWithScroll = function(obj) {
	...
	if (obj.scrollHeight>obj.clientHeight) return obj;
...
yoodoo.formatDate = function(f, dt) {
	...
	if ( typeof (dt.getDate) == "function") {
		...
		var days = [];
		for(var i=0;i<yoodoo.days.length;i++) days.push(yoodoo.w(yoodoo.days[i]));
		var months = [];
		for(var i=0;i<yoodoo.months.length;i++) months.push(yoodoo.w(yoodoo.months[i]));
