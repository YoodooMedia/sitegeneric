...
console.log(ft);
	...
	saveSnapshot: function () {
		if (this.canSnapshot) {
			...
			var p = {};
			var data=dooit.values({encode:false}); // do not base64 encode
			var vals=[];
			for(var k in data) {
				var i=k.replace(/^EF/,'');
	...
	loadSnapshot:function(snapshotId) {
		...
		yoodoo.hide(function() {
			var thisId=yoodoo.lastLoad;
			yoodoo.lastLoad='';
			yoodoo.showDooit(thisId,dooit.fetchingSnapshotId);
		});
	},
	...
	ready:function() {
		dooit.loaded();
		yoodoo.displayDooit();
		if (!dooit.canSnapshot) $('.scrapbookOptions #scrapbookaddsnapshot').remove();
		dooit.displayed();
		...
		}else if(dooitInformation.snapshot!="{}") {
