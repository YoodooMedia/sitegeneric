...
yoodoo.progress = function() {
	...
	//console.log(ni,this.autoProgress);
	if (ni !== null && this.autoProgress) {
		var autoPlay=false;
		if (ni.autoplay===true) autoPlay=!ni.completed;
		if (this.intervention!==null) autoPlay=true;
		//if (ni.autoplay && !this.hasTag("autoplayOff")) {
			//console.log(ni);
			this.gotoItem(ni,autoPlay);
			/*if (this.html5) {
...
yoodoo.gotXML = function(html) {
	...
	if (this.bookcaseFirstLoad && this.intervention === null) {
		/*var i=0;
...
yoodoo.gotoItem = function(item) {
	...
	if (yoodoo.html5) {
		...
		if (autoPlay && item.type=='book') {
			yoodoo.startEpisode(item.id);
		}else{
			yoodoo.callFlash(yoodoo.cmd.scrollTo.flash, item.xmlindex+1);
...
yoodoo.processFilterBookcase = function() {
	...
	for (var i = 0; i < this.bookcase.items.length; i++) {
		...
		if (this.intervention!==null) {
			...
			if (this.bookcase.items[i].visible===true) {
				if (hideRest) {
			...
			if (!this.bookcase.items[i].visible) {
				removeFromXML[this.bookcase.items[i].type].push(this.bookcase.items[i].id);
				this.bookcase.items[i].xmlindex=null;
			}else{
				this.bookcase.items[i].xmlindex=idx++;
			}
			...
			if (!hideRest) {
				if (yoodoo.intervention===null && this.bookcase.items[i].hideAfterTilComplete && this.bookcase.items[i].completed && !(this.intervention>0 && this.bookcase.items[i].intervention_exclude!==true) && this.bookcase.items[i].completed_date.getTime!==undefined) {
					...
					if (this.bookcase.items[i].hideAfterTilCompleteHours>0) {
						...
						// hack reveal by minutes
							completedTime=new Date(completedTime.getFullYear(),completedTime.getMonth(),completedTime.getDate(),completedTime.getHours(),completedTime.getMinutes()+this.revealMinutes,completedTime.getSeconds());
						}else{
				...
				if (previousCompleted && this.bookcase.items[i].completed_date!==undefined) {
	...
	var idx=-1;
	if (this.nextItem!==null && this.bookcaseFirstLoad) idx=this.nextItem.xmlindex+1;
	if (this.nextReveal!==null && this.bookcaseFirstLoad) {
		this.nextItem=this.lastCompleteItem();
		if (this.nextItem!==null) idx=this.nextItem.xmlindex+1;
	}
	if (!this.html5) {
		...
		yoodoo.callFlash(yoodoo.cmd.bookshelfxml.flash, txt, idx);
	} else {
		var currentIndex=(this.nextItem!==null)?this.nextItem.index:-1;
		if (yoodoo.fetchedItem!==undefined) {
			var i = this.bookcaseIndex(this.fetchedItem.type, this.fetchedItem.id);
			if (i>=0 && this.bookcase.items[i].visible) currentIndex=this.bookcase.items[i].index;
		}
		yoodooPlaya.bookcaseLoaded(currentIndex);
	}
};
...
yoodoo.lastCompleteItem = function() {
	var i=this.bookcase.items.length-1;
	while(i>=0 && (this.bookcase.items[i].visible===false || this.bookcase.items[i].completed===false)) i--;
	if (i>=0 && this.bookcase.items.length>0 && this.bookcase.items[i].visible===true) {
		return this.bookcase.items[i];
	}else{
		return null;
	}
