...
console.log(ft);
		...
		if (typeof(yoodoo.interface)=='object' && yoodoo.metaphor==2 && opts.orientation!==undefined) yoodoo.interface.setOrientation(opts.orientation);
		...
		if (typeof(opts.dooit)=='string') {
			var obj={};
				dooit.temporaries(opts.dooit);
				if (opts.displayed===undefined) opts.displayed=opts.dooit+'.displayed';
				if (opts.saveValues===undefined) opts.saveValues=[opts.dooit+'.output'];
				if (opts.finished===undefined) opts.finished=opts.dooit+'.finishable';
				if (opts.presave===undefined) opts.presave=opts.dooit+'.presave';
		}
		...
		if (opts.presave!==undefined) {
			eval('this.presave=function() {if ('+opts.presave+'!==undefined) {return '+opts.presave+'();}else{return true;}};');
		}else{
			this.presave=function(){};
		}
		var dependencies=[];
		if(opts.dependencies!==undefined) dependencies=opts.dependencies;
		...
		this.loaded=function() {};
		if (opts.loaded!==undefined) this.loaded=opts.loaded;
		if (opts.done!==undefined) this.done=opts.done;
		if (opts.saveValues!==undefined) this.saveValues=opts.saveValues;
		if (opts.noscroll!==undefined) this.noscroll=opts.noscroll;
		if (opts.close!==undefined) this.onclose=opts.close;
		this.params=opts;
		this.options={};
		if (opts.options!==undefined) this.options=opts.options;
		
		if (this.noscroll) $('#yoodooScrolledArea').css({overflow:"hidden"});
		yoodoo.dooitFiles=new yoodoo.fileLoader.loader(dependencies,function(complete) {
			if (complete) {
				dooit.ready();
			}else{
				dooit.failed();
	...
	createSnapshot:function (callback) {
		if (this.canSnapshot) {
			...
			//var dooit=yoodoo.bookcase.items[yoodoo.bookcaseIndex('dooit',yoodoo.lastLoad)];
			var dooit=yoodoo.bookcase.fetchDooit(yoodoo.lastLoad);
			var p = {};
			p.data=this.snapshotData();
			p.contentid = dooit.content_id;
	...
	ready:function() {
		if (typeof(this.params.dooit)=='string') {
			var obj={};
			try{
				eval('obj='+this.params.dooit+';');
				yoodoo.dooit_standardize(obj);
				if (this.params.loaded===undefined) this.loaded=function() {obj.init();};
			}catch(e){}
		}	
