var personal_strengths={
	...
	init:function(key) {
		...
		$(this.container).html('<h2>Your Strengths and Weaknesses</h2><div class="teaser">Help find out what you&apos;re best as - and the things to avoid</div>');
	...
	warning:function() {
		...
		if (this.strengthPercentage<30) {
			...
			if (this.items.length<7) {
				ins="Are really so perfect? Be realistic about your Weaknesses.";
			}else{
	...
	listAvailable:function() {
		var ins='';
		for(var s=0;s<strengths.length;s++) {
			if (this.isSelected(strengths[s])<0) ins+='<div class="suggestion strong">'+strengths[s]+'</div>';
		}
		$('#availableStrengths').html(ins);
		$('#availableStrengths>div').bind('click',function() {
			if (personal_strengths.items.length<10) {
				personal_strengths.addStrength(this.innerHTML);
		...
		ins='';
		for(var s=0;s<weaknesses.length;s++) {
			if (this.isSelected(weaknesses[s])<0) ins+='<div class="suggestion weak">'+weaknesses[s]+'</div>';
		}
		$('#availableWeaknesses').html(ins);
		$('#availableWeaknesses>div').bind('click',function() {
			if (personal_strengths.items.length<10) {
				personal_strengths.addWeakness(this.innerHTML);
					$(this).animate({height:'0px'},function() {
						$(this).remove();
					});
			}else{
