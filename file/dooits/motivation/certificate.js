...
var certificate={
	...
	init:function() {
		...
		this.loadFields();
		this.containers.container=$(this.selectors.container).empty();
		this.render();
	},
	loaded:function() {
		...
		$(certificate.containers.previewContainer).addClass("previewContainer");
	...
	render:function() {
		...
		$(this.containers.container).append(title).append(description);
	...
	getPDF:function() {
		...
		yoodoo.toPDF({orientation:certificate.value.main.orientation.substring(0,1),header:header,content:content},certificate.value.main.title.replace(/\{user\}/g,certificate.username));
	},
	preview:function() {
