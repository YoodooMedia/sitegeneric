...
//console.log(ans);
  ...
  input:((keycode==190)||(keycode==188)||(keycode==192)||(keycode==111)||(keycode==192)||(keycode==191)||(keycode==107)||(keycode==187)||(keycode==189)||(keycode==106)||(keycode==110)||(keycode==220)||(keycode==223)||(keycode==222)||(keycode==221)||(keycode==219)||(keycode==186)),
	...
	render:function() {
		...
		op+="<button type='button' class='button toggle green' onclick='"+(this.reporting?'display_dooit.output();':'')+"display_dooit.toggle();'>"+(this.reporting?'edit':'report')+"</button>";
		op+="<div class='fields'>";		
...
//console.log(pos);
	...
	finishable:function() {
		var ok=true;
		for(var i=0;i<this.data.structure.length;i++) {
			if (this.data.structure[i].value.required=="1") {
				var id=this.data.structure[i].id;
				switch(this.data.structure[i].value.type) {
					case "paragraph":
						break;
					case "text":
						for(var j=0;j<this.data.structure[i].value.option.length;j++) {
							if (this.value[id][j]=='') ok=false;
						}
						break;
					case "numeric":
						for(var j=0;j<this.data.structure[i].value.option.length;j++) {
					...
					case "checkbox":
						var checked=false;
						for(var j=0;j<this.data.structure[i].value.option.length;j++) {
							if (this.value[id][j]=='1') checked=true;
						}
						if (!checked) ok=false;
						break;
					case "select":
						var checked=false;
						for(var j=0;j<this.data.structure[i].value.option.length;j++) {
							if (this.value[d][j]=='1') checked=true;
						}
						if (!checked) ok=false;
						break;
				}
	...
	scoring:{
		...
		rag_balloons:{
			...
			top_height:0,
			init:function(){
				var h=$('#yoodooScrolledArea').height()-$(display_dooit.container).find('.fields').offset().top;
				...
				fields.parentNode.insertBefore(this.frame,fields);
			...
			process:function() {
				if($('.rag_balloons .red').get().length>0) {
					...
					setTimeout('display_dooit.score_system.process();',20);
				}
			}
		},
		galileo:{
