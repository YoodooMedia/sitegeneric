...
/*	type specific functions	*/
	...
	field_summary:function(d) {
		...
		switch(this.data.structure[d].value.type) {
			case "paragraph":
				...
				var summary=((this.data.structure[d].title=='')?'Undefined':sumText);
				if (content_only) {
					return summary;
				}else{					
			...
			break;
			default:
				var summary=((this.data.structure[d].value.grouped=='1')?'Grouped with the above':((this.data.structure[d].title=='')?'Undefined':constructor_dooit.summary_text(this.data.structure[d].title)));
	...
	field_editor:function(d) {
		...
		switch(this.data.structure[d].value.type) {
			case "paragraph":
				op+="<div class='field_info'>"+ins+"<div id='paragraph"+d+"' class='paragraph' onclick='constructor_dooit.edit_paragraph(this,"+d+")''>"+this.data.structure[d].title+"</div>"+this.type_selector(this.data.structure[d].value.type)+this.type_help[this.data.structure[d].value.type]()+"</div>";
			break;
			case "slider":
				...
				op+="Title: <input type='text' value='"+this.data.structure[d].title+"' onkeyup='constructor_dooit.edit_title(this);' />"+this.type_selector(this.data.structure[d].value.type)+this.type_help[this.data.structure[d].value.type]()+"</div>";
...
//console.log(constructor_dooit.attachStructure[refs[ref][r]].value.option);
	...
	type_option:{
		...
		text:function(val) {
			var ins="<div class='move movevalue'></div><input type='text' value='"+val.title+"' onkeyup='constructor_dooit.change(\"title\",this)' />";
...
//console.log(o.parentNode.parentNode.parentNode);
			if(checked) {
				...
				$(o).removeClass("on");
				$(o).html("ungrouped");
				$(o.parentNode.parentNode.parentNode).find('.summary .field_summary').html(constructor_dooit.data.structure[i].title);
			}
	...
	field:function() {
		return {title:'Title',required:'0',id:this.uniqueID(),value:{type:'text',grouped:'0',multiple:'0', option:[]}};
	},
	uniqueID:function() {
	...
	render:function() {
		...
		op+="<div class='info'><label>Dooit title:</label><input type='text' value='"+this.data.dooit.title+"' onkeyup='constructor_dooit.data.dooit.title=this.value;' /></div>";
		op+="<div class='info'><label>Subheading:</label><input type='text' value='"+this.data.dooit.subheading+"' onkeyup='constructor_dooit.data.dooit.subheading=this.value;' /></div>";
	...
	field_container:function(d) {
		...
		var ins="<button type='button' class='removeButton' onclick='constructor_dooit.remove(this.parentNode);' style='float:right'></button>";
		ins+="<button type='button' class='copyButton' onclick='constructor_dooit.copy(this.parentNode);' style='float:right'></button>";
	...
	edit_paragraph:function(o,d) {
		...
		o.parentNode.parentNode.rte=this.nic.panelInstance(o.id,{hasPanel:true});
		o.parentNode.parentNode.rteid=o.id;
		o.parentNode.parentNode.question=d;
		o.parentNode.parentNode.field_name='title';
		if ($(o).hasClass("head")) o.parentNode.parentNode.field_name='paragraph';
	},
		...
		if (this.open_editor!==null) {
			if (this.open_editor.rteid!==null && this.nic!==null) {
				...
				if (typeof(inst)!="undefined") {
					this.data.structure[this.open_editor.question][this.open_editor.field_name]=inst.getContent();
	...
	add:function() {
		...
		//if (this.data.structure.length>0) {
			...
			val.value.option=[];
			for(var i=0;i<this.data.structure[idx].value.option.length;i++) val.value.option.push(this.data.structure[idx].value.option[i]);
		}else{
			if (this.data.structure.length>0) {
