...
var kloe = {
	...
	init: function() {
		...
		this.loadFields();
		this.containers.container = $(this.selectors.container).addClass("kloe").append(
						...
						yoodoo.object.get(me.otherObjectType, function(obj) {
							...
							yoodoo.object.get(me.objectType, function(obj) {
								...
								if (obj.length == 1) {
									...
									for (var k in me.schema.sections) {
										me.standardizeSection(me.schema.sections[k]);
									}

									//console.log(obj,obj.length);
									var filter = {};
									filter[me.object.getParameterReferingToObjectId(yoodoo.businessSector.object.schema.Id)] = [me.businessSector.Id, 'equals'];
									filter[me.object.getParameterReferingToObjectId(me.objectTypes[me.objectName.keyQuestions].schema.Id)] = [me.schema.keyQuestion, 'equals'];
									...
									if (me.upgraded !== true) {
										...
										for (var s in me.schema.sections) {
											...
											for (var i in me.schema.sections[s].items) {
												...
												me.shouldUpgrade += me.schema.sections[s].items[i].records.length;
											}
										}
										me.shouldUpgrade = (me.shouldUpgrade > 0);
										if (me.shouldUpgrade === false) me.upgraded = true;
									}
										...
										me.object.get(function(list) {
											...
											for (var l in list) {
												...
												if (list[l].readonly) {
													...
													list[l].itemObject=kloe.prompts[list[l].value[subsectionParameter]];
													templates[list[l].Id] = list[l];
												} else {
													if (kloe.prompts[list[l].value[subsectionParameter]] !== undefined) {
														var row = new kloe_response.record(list[l],kloe.prompts[list[l].value[subsectionParameter]], kloe.templater);
														responses.push(row);
											...
											//console.log(list);
											//if (kloe.upgraded === true && kloe.templater!==true) {
												kloe_response.createInherited(templates,responses,null,function(optional) {
													...
													for(var p in kloe.prompts) kloe.prompts[p].optional=[];
													for(var o in optional) {
														kloe.prompts[optional[o].value[subsectionParameter]].optional.push(optional[o]);
													}
													yoodoo.working(false);
													me.start();
												},function(cloned) {
													//console.log(cloned);
													kloe.prompts[cloned.object.value[subsectionParameter]].records.push(cloned);
												});
			...
			if (window.confirm("You need to upgrade your data to the new system. Click OK to upgrade now.")) {
				...
				for (var s in this.schema.sections) {
					...
					for (var i in this.schema.sections[s].items) {
						...
						//console.log(this.schema.sections[s].items[i].kloeSubSection);
						...
						for (var r in this.schema.sections[s].items[i].records) {
							...
							newRecord.setValue(this.schema.textField, {
								hklsj: oldRecord.text
							});
							...
							if (typeof(oldRecord.doc) == "string" && oldRecord.doc !== "") {
								...
								if (typeof(oldRecord.file) == 'object' && oldRecord.file !== null && oldRecord.file.length === undefined) {
									...
									for (var i in jsonSchema) {
										var nom = jsonSchema[i].title.en.toLowerCase();
										if (oldRecord.file[nom] !== undefined && oldRecord.file[nom] !== null) {
											op[jsonSchema[i].key] = oldRecord.file[nom];
										}
									}
	...
	standardizeSection: function(obj) {
		...
		obj.render = function() {
			if (kloe.templater) {
				...
				this.button.append(
					$(yoodoo.e("div")).addClass("immediateItems").append(kloe.icon(this.context.icons.cross, 20, 20, 100, 100, {
						'4D4D4D': this.context.colours.red
					})).append(this.immediateItems)
				).append(
					$(yoodoo.e("div")).addClass("imminentItems").append(kloe.icon(this.context.icons.tick, 20, 20, 100, 100, {
						'4D4D4D': this.context.colours.amber
					})).append(this.imminentItems)
				).append(
					$(yoodoo.e("div")).addClass("actionplans").append(kloe.icon(this.context.icons.walking, 20, 20, 100, 100, {
						'4D4D4D': this.context.colours.cyan
					})).append(this.actionplansItems)
				).append(
					$(yoodoo.e("div")).addClass("exclamations").append(kloe.icon(this.context.icons.exclamation, 20, 20, 100, 100, {
		...
		obj.open = function() {
			if (kloe.templater!==true) this.minibar = {
				container: $(yoodoo.e("div")).addClass("scoreBar"),
				bars: yoodoo.e("div"),
				render: function(val) {
			...
			var back = $(yoodoo.e("button")).attr("type", "button").addClass("backButton").click(function() {
				me.show(null);
			}).html(this.context.schema.title).prepend(kloe.icon(this.context.icons.left, 20, 20, 100, 100, {
				'4D4D4D': 'FFFFFF'
			}));
			...
			me.containers.items.empty().append(
				kloe.templater?null:this.minibar.render()
			).append(
				back
			).append(
			...
			if (kloe.templater!==true) this.minibar.update(this.score());
			me.show(true);
		};
		obj.responseCount = function() {
			var count=0;
	...
	standardizeItem: function(obj) {
		...
		/*if (kloe.upgraded !== true) {
			for (var r = 0; r < this.value[obj.key].length; r++)
				obj.records.push(new kloe_response.record(obj, this.value[obj.key][r],kloe.templater));
		}*/
		if (obj.kloeSubSection > 0 && this.objectTypes[this.objectName.subSections] !== null && this.objectTypes[this.objectName.subSections].recordsCache[obj.kloeSubSection] !== undefined) {
			...
			obj.title=obj.prompt.value.lvzfz;
		...
		obj.add = function(vals) {
			...
			if (kloe.upgraded) {
				...
				item = new kloe_response.record(newRecord,this,kloe.templater);
			} else {
				item = new kloe_response.record(vals,this,kloe.templater);
			}
		...
		obj.update = function() {
			...
			if (kloe.templater) {
				...
				this.actionPlanIcon.empty().append($(kloe.icon(this.context.icons.walking, 30, 30, 100, 100, rep)).css({
		...
		obj.open = function() {
			...
			var back = $(yoodoo.e("button")).attr("type", "button").addClass("backButton").click(function() {
				...
				kloe.icon(this.context.icons.left, 20, 20, 100, 100, {
					'4D4D4D': 'FFFFFF'
				})
			);
			...
			if (this.optional instanceof Array && this.optional.length>0) {
					...
					(function() {
						...
						prompt.optionalList.append(
							$(yoodoo.e("button")).attr("type","button").html(optionalTemplate.displayName()).append(
								...
								var item=new kloe_response.record(newRecord,prompt,kloe.templater);
				...
				me.containers.records.append(this.optionalList);
			}
		};
	...
	help: function(text, sound) {
		...
		var blockout = $(yoodoo.e("div")).addClass("blockout").click(function(e) {
			...
			$(yoodoo.e("button")).attr("type","button").html("Open in a new window").click(function() {
				var win=window.open(null,'expertinsight','menubar=no,status=no,toolbar=no,location=no,titlebar=no,width='+yoodoo.option.width+',height='+yoodoo.option.height);
				...
				//win.document.write('<html><head><title>Expert Insight</title></head><body>'+helpMessage.html()+'</body></html>');
			})
		);
