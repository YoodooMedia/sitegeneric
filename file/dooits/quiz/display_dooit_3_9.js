...
//console.log(i,this.auto_move[this.data.structure[i].value.type],canProgress);
	...
	getQuestionScores:function(i) {
		if (this.data.structure.length<i) return {scores:{},labels:{}};
		...
		for(var a=0;a<maxAnswers;a++) {
			for(var di=0;di<d.length;di++) {
				if (this.data.structure[d[di]].value.type=="select" || this.data.structure[d[di]].value.type=="checkbox") {
					if (this.data.structure[d[di]].value.option.length>0 && this.data.structure[d[di]].value.option.length==this.value[this.data.structure[d[di]].id][a].length) {
						var selected=false;
