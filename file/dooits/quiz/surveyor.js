...
var surveyor={
	...
	build:function() {
		...
		var ins='';
		if(this.structure.titleTag!=undefined) {
			ins+="<"+this.structure.titleTag+">"+this.structure.title+"</"+this.structure.titleTag+">";
		}else{
			ins+="<h2>"+this.structure.title+"</h2>";
		}
		ins+="<div class='scroller'>";
		var tempValue=[];
		if(this.value.length==this.structure.questions.length && typeof(this.value[0])=="string") {
			tempValue=this.value;
			this.value=[];
		}
		for(var i=0;i<this.structure.questions.length;i++) {
			var d=this.structure.divisions;
			if (this.structure.questions[i].divisions!=undefined) d=this.structure.questions[i].divisions;
			var low=this.structure.minText;
			if (this.structure.questions[i].minText!=undefined) low=this.structure.questions[i].minText;
			var high=this.structure.maxText;
			if (this.structure.questions[i].maxText!=undefined) high=this.structure.questions[i].maxText;
			if (this.value.length<=i) {
				this.value.push({q:this.structure.questions[i].question,d:d,low:low,high:high,a:undefined});
			}else if( typeof(this.value[i].q)=="undefined") {
				this.value[i]={q:this.structure.questions[i].question,d:d,low:low,high:high,a:undefined};
			}
			if (typeof(tempValue[i])=="string") this.value[i].a=parseInt(tempValue[i]);
		...
		var ec=$('#exercise_content');
		if (ysa!==null) {
			this.scroller=$(this.container).find('.scroller').get(0);
			//ysa=ysa[0];
			var topOffset=$(this.scroller).offset().top-ec.offset().top;
//console.log(ec.offset().top,$(this.scroller).offset().top,ysa.height());
			var h=ysa.height()-topOffset-10;
	...
	slider:function(i) {
		...
		var ins="<div class='slider'><div class='surveyheader'><div class='mintext'>"+mint+"</div><div class='maxtext'>"+maxt+"</div></div><div style='clear:both'></div>";
