...
//'"'+o.replace(/"/g,'\\"')+'"';
	...
	tag_rule_editor:{
		...
		rule_item:function(r) {
			...
			if (constructor_dooit.data.dooit.tag_rules.rules[r].unset==undefined) constructor_dooit.data.dooit.tag_rules.rules[r].unset=true;
			...
			ins+='&nbsp;&nbsp;<span>Below: <input type="checkbox" '+(constructor_dooit.data.dooit.tag_rules.rules[r].below?' checked':'')+' onchange="constructor_dooit.tag_rule_editor.update_rule(\'below\',this)"/> <input type="text" onchange="constructor_dooit.tag_rule_editor.update_rule(\'abovePercent\',this)" value="'+constructor_dooit.data.dooit.tag_rules.rules[r].belowPercent+'" style="width:30px;display:'+(constructor_dooit.data.dooit.tag_rules.rules[r].below?'inline':'none')+'" />%</span>';
			$(rule).html(ins);
			return rule;
		},
		update_rule:function(t,o) {
			var i=$(constructor_dooit.parent_of_class(o,'rule_item')).prevAll('.rule_item').get().length;
			if (t=="highest" || t=="lowest" || t=="unique" || t=="compare" || t=="above" || t=="below" || t=="set") {
