...
var ppa_profile={
	...
	processCompletion:function() {
		...
		$(this.containers.area).animate({top:-10,opacity:0},this.animateDuration,function() {
			$(this).html("<div class='processing'>Fetching your results</div>").animate({top:0,opacity:1},ppa_profile.animateDuration,function() {
				var toSend={refId:ppa_profile.refId(),justData:justData,data:ppa_profile.value.responses};
	...
	analyze:function() {
		/*this.value.ppa.traits={};
			this.value.ppa.traits.invalid=this.check.invalid();
			this.value.ppa.traits.tight1=this.check.tight([true,false,false]);
			this.value.ppa.traits.tight2=this.check.tight([false,true,false]);
			this.value.ppa.traits.tight3=this.check.tight([false,false,true]);
			this.value.ppa.traits.tightAll=this.check.tight([true,true,true]);
			this.value.ppa.traits.DequalsC=this.check.DequalsC(); // [1,0,-1] 1=equal above the line, -1=equal below the line
			this.value.ppa.traits.overshift=this.check.overshift();
			this.value.ppa.traits.undershift=this.check.undershift();
			this.value.ppa.traits.greyZone=this.check.greyZone();
			this.value.ppa.traits.Smovement=this.check.Smovement();
			this.value.ppa.traits.flickUp=this.check.flickUp();
			this.value.ppa.traits.flickDown=this.check.flickDown();
			this.value.ppa.traits.sweepDown=this.check.sweepDown();
			...
			this.value.ppa.traits.readOrder=this.check.readOrder();
...
console.log(this.value.ppa.traits);
		for(var t=0;t<this.structure.main.optionTags.length;t++) {
			...
			if (tag.tag!="") {
				if (tag.characteristic!="none") {
					on=(this.value.ppa.traits.basicFactorCombinaton.combination.replace('/','')==tag.characteristic);
				}
				if (tag.trait!="none") {
					switch(tag.trait) {
						case 'greyZone':
	...
	check:{
		...
		highD:function() {
			var a=true;
			if (ppa_profile.value.ppa.graphs[2].D<ppa_profile.value.ppa.graphs[2].S) a=false;
			if (ppa_profile.value.ppa.graphs[2].D<ppa_profile.value.ppa.graphs[2].I) a=false;
			if (ppa_profile.value.ppa.graphs[2].D<ppa_profile.value.ppa.graphs[2].C) a=false;
			return a;
		},
		highI:function() {
			var a=true;
			if (ppa_profile.value.ppa.graphs[2].I<ppa_profile.value.ppa.graphs[2].S) a=false;
			if (ppa_profile.value.ppa.graphs[2].I<ppa_profile.value.ppa.graphs[2].D) a=false;
			if (ppa_profile.value.ppa.graphs[2].I<ppa_profile.value.ppa.graphs[2].C) a=false;
			return a;
		},
		highS:function() {
			var a=true;
			if (ppa_profile.value.ppa.graphs[2].S<ppa_profile.value.ppa.graphs[2].D) a=false;
			if (ppa_profile.value.ppa.graphs[2].S<ppa_profile.value.ppa.graphs[2].I) a=false;
			if (ppa_profile.value.ppa.graphs[2].S<ppa_profile.value.ppa.graphs[2].C) a=false;
			return a;
		},
		highC:function() {
			var a=true;
			if (ppa_profile.value.ppa.graphs[2].C<ppa_profile.value.ppa.graphs[2].D) a=false;
			if (ppa_profile.value.ppa.graphs[2].C<ppa_profile.value.ppa.graphs[2].I) a=false;
			if (ppa_profile.value.ppa.graphs[2].C<ppa_profile.value.ppa.graphs[2].S) a=false;
			return a;
		},
		readOrder:function() {
			var above=[];
		...
		basicFactorCombination:function() {
			...
			for(var k in ppa_profile.value.ppa.graphs[2]) {
				if (highest=='') {
					highest=k;
				}else if(ppa_profile.value.ppa.graphs[2][k]>ppa_profile.value.ppa.graphs[2][highest]){
					highest=k;
				}
				if (lowest=='') {
					lowest=k;
				}else if(ppa_profile.value.ppa.graphs[2][k]<ppa_profile.value.ppa.graphs[2][lowest]){
					lowest=k;
				}
			}
