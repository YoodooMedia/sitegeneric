...
var boxdrop={
	...
	complete:function() {
		...
		var but=yoodoo.e("button");
		$(but).attr("type","button");
		$(but).html(this.structure.main.restart).addClass("boxdropRestartButton").bind("click",function() {boxdrop.nextQuestion();});
	...
	renderQuestion:function() {
		...
		for(var b=0;b<boxes.length;b++) {
			...
			box.box=boxes[b];
			box.isCorrect=(q.correct=='' || q.correct==boxes[b].id);
			if (q.correct=='') box.noCorrect=true;
			...
			box.correct=function() {
				...
				if (this.noCorrect===true) return this.processNoCorrect();
