...
var quizit={
	...
	init:function() {
		...
		if (this.key!==null && this.containers.container!==null) {
			...
			this.containers.container.html("<h2>"+this.structure.title+"</h2><div class='quizit_header'><div class='quizit_progress'><div></div>&nbsp;</div>&nbsp;<div class='quizit_timer_area'><span>No started</span></div></div><div class='quizit_display_area'></div>");
	...
	pageObject:function(i) {
		...
		this.questions=[];
//console.log(this.index,this.page,quizit.structure.pages[i]);
		...
		this.render=function() {
			...
			for(var q=0;q<quizit.value.pages[this.index].questions.length;q++) {
				...
				this.questions[tq].render(quizit.containers.pages.get(0));
			}
			var vp=document.createElement("button");
			$(vp).attr('type','button');
			vp.innerHTML=(quizit.page<quizit.structure.pages.length-1)?quizit.structure.nextPage:quizit.structure.finish;
			$(vp).bind("click",function() {
				$(this).unbind("click");
				quizit.nextPage();
			}).addClass('yd_cta').css({float:'right'});
	...
	question:function(q) {
		...
		this.render=function(o) {
			this.container=o;
			this.container.appendChild(this.object.render());
		}
	},
	questionTypes:{
		select:function(q) {
		...
		text:function(q) {
			...
			this.answered=function(ans) {
				...
				if (ans=="") {
					...
					if (this.question.settings.caseInsensative) txt=txt.toLowerCase();
					for(var m=0;m<this.options.length;m++) {
						var toCheck=this.options[m].title.replace(/^ +/,'').replace(/ +$/,'');
						if (this.question.settings.caseInsensative) toCheck=toCheck.toLowerCase();
						if (txt==toCheck) {
