	...
	type_output:{
		...
		text:function(d,val,a) {
			...
			if (val.option.length==0) {
				for(var i=0;i<val.option.length;i++) ins+="<div class='item2'><input type='text' onkeyup='display_dooit.set_value(\""+d+"\","+i+",this.value,"+a+");' value='"+(display_dooit.value[d][a][i]?dooit.encode(display_dooit.value[d][a][i]):"")+"' /></div>";
			}else{
				for(var i=0;i<val.option.length;i++) ins+="<div class='item'><span class='label'>"+val.option[i].title+"</span><input type='text' onkeyup='display_dooit.set_value(\""+d+"\","+i+",this.value,"+a+");' value='"+(display_dooit.value[d][a][i]?dooit.encode(display_dooit.value[d][a][i]):"")+"' /></div>";
			}
			return ins;
		},
...
//console.log(i);
	...
	render:function() {
		...
		if (!this.data.dooit.serialize && !this.onlyReporting) op+="<button type='button' class='button toggle green' onclick='"+(this.reporting?'display_dooit.output();':'')+"display_dooit.toggle();'>"+(this.reporting?'edit':'report')+"</button>";
		if (this.reporting) {
...
//console.log('Required',id,display_dooit.value[id],ok);
	...
	scoring:{
		...
		multi_balloons:{
			...
			update:function() {
				this.scores={'red':0,'amber':0,'green':0,'purple':0,'yellow':0,'blue':0};
				var quantity=0;
				for(var q=0;q<display_dooit.data.structure.length;q++) {
					if(typeof(display_dooit.value[display_dooit.data.structure[q].id])!="undefined") {
						var answers=1;
