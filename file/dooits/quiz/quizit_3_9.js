...
var quizit={
	...
	init:function() {
		...
		if (this.key!==null && this.containers.container!==null) {
			...
			for(var p=0;p<this.structure.pages.length;p++) {
				...
				this.pages[this.structure.pages[p].id]=this.structure.pages[p];
			}
			this.checkUser();
		}
	},
	displayed:function() {
		...
		yoodoo.closeDooitFunction=function() {
			if (quizit!==undefined)	quizit.timer.cancel();
			yoodoo.closeDooitFunction=function() {};
		};
	},
	showStart:function() {
		quizit.completed=false;
		quizit.canAutoProgress=true;
	...
	pageObject:function(i) {
		...
		this.defineUserField=function() {
			...
			for(var q=0;q<reply.questions.length;q++) {
				var ms=0;
//console.log(this.pageInfo.questions[reply.questions[q].index].type);
				if (this.pageInfo.questions[reply.questions[q].index].type=="Reorder") {
					if (this.pageInfo.questions[reply.questions[q].index].position>0) {
						for(var o=0;o<this.pageInfo.questions[reply.questions[q].index].options.length;o++) {
							if (o<this.pageInfo.questions[reply.questions[q].index].position) {
						...
						//}
					//}
					if (quizit.isNotEmpty(this.pageInfo.questions[reply.questions[q].index].scoreall)) {
						if (this.pageInfo.questions[reply.questions[q].index].scoreall>ms) {
							ms=parseInt(this.pageInfo.questions[reply.questions[q].index].scoreall);
						}
					}
...
//console.log(reply.questions[q].answer);
				}
		...
		this.autoComplete=function() {
			...
			if (this.pageInfo.autoProgress===true && this.pageInfo.serial===true) {
				if ($(quizit.answeredQuestion.object.container).hasClass('questionCompleted')) {
					if ($(quizit.answeredQuestion.object.container).hasClass('questionRequiresConfirmation')) {
						$(this.nextButton).stop().slideDown();
						return false;
					}else{
						$(this.nextButton).stop().slideUp();
						return true;
					}
				}else{
					$(this.nextButton).stop().slideUp();
			...
			}else if (this.pageInfo.autoProgress===true) {
				var complete=$(quizit.containers.pages).find('.quizit_question.questionCompleted').get().length;
				var total=$(quizit.containers.pages).find('.quizit_question').get().length;
				var confirmation=$(quizit.containers.pages).find('.quizit_question.questionRequiresConfirmation').get().length;
				if (confirmation>0) {
						$(this.nextButton).slideDown();
						return false;
				}else{
					$(this.nextButton).stop().slideUp();
		...
		this.render=function() {
			...
			if (this.pageInfo.autoProgress===true) {
				if (this.pageInfo.serial===true) {
					if (!(quizit.containers.pages.find('.quizit_question').first().hasClass('questionRequiresConfirmation'))) {
						$(vp).hide();
					}
				}else{
					if (quizit.containers.pages.find('.quizit_question.questionRequiresConfirmation').get().length==0) $(vp).hide();
				}
			}
			...
			if (quizit.page>0 && quizit.navigable) {
				...
				$(vp).bind("click",function() {
					$(this).unbind("click");
					quizit.prevPage();
				}).addClass('yd_cta').css({float:'left'});
				quizit.containers.pages.get(0).appendChild(vp);
			}
			...
			yoodoo.stopSound();
			if (this.pageInfo.serial!==true && this.pageInfo.sound!==undefined && this.pageInfo.sound.url!="") {
				yoodoo.playSound(yoodoo.option.baseUrl+this.pageInfo.sound.url);
			}else if (this.pageInfo.serial===true && this.questions[0].settings.sound!==undefined && this.questions[0].settings.sound.url!="") {
				yoodoo.playSound(yoodoo.option.baseUrl+this.questions[0].settings.sound.url);
			}

			if (quizit.sequence<this.questions.length) {
				
				var q=this.questions[quizit.sequence];
				quizit.answeredQuestion=q;
			}
		};
	},
	question:function(q) {
		...
		this.respond=function() {
			...
			var res=false;
//console.log('Respond object',this.object);
//console.log('Respond object response',this.object.response);
			if (this.settings.responses!==undefined) {
				for(var r=0;r<this.settings.responses.length;r++) {
//console.log('Response id',this.settings.responses[r].id);
...
//console.log(this.nextResponse);
		...
		this.protect=function() {
			...
			$(this.hide).css({position:"absolute",width:'100%',height:'100%',padding:0,top:0,left:0}).addClass('hideAndProtect');
			$(this.object.container).css({position:"relative"}).append(this.hide);
			this.responseDiv=document.createElement("div");
			$(this.responseDiv).css({'font-weight':'bold',width:$(this.hide).width()-20,height:$(this.hide).height()-20,padding:10,'text-align':'center'});
			$(this.hide).append(this.responseDiv);
			//console.log(this.object.container);
		};
		this.revealResponse=function() {
			yoodoo.stopSound();
			$(this.responseDiv).html(this.responseObject.text).css({color:'#'+this.responseObject.colour+' !important'}).find('p').css({color:'#'+this.responseObject.colour+' !important'});
	...
	questionTypes:{
		select:function(q) {
			...
			this.container=null;
			this.response=null;
			this.getResponse=function(k) {
				for (var r=0;r<this.question.settings.responses.length;r++) {
					if (this.question.settings.responses[r].id==k) return this.question.settings.responses[r];
				}
				return null;
			};
			this.render=function(o) {
...
//console.log('Answer',ans.answer);
				quizit.answeredQuestion=this.question;
				...
				this.response=ans.answer.response;
				if (this.response==null && typeof(this.question.settings.response)=="string") {
					this.response=this.question.settings.response;
				}
//console.log('Answered',this.response,this);
				quizit.updateStatus();
				$(this.container).addClass("questionCompleted");
				if (quizit.structure.pages[quizit.page].object.autoComplete()) quizit.completeQuestion();
		...
		multiplechoice:function(q) {
			...
			this.paragraph=quizit.isNotEmpty(q.settings.paragraph)?'<p class="question_paragraph">'+q.settings.paragraph+'</p>':'';
			this.container=null;
			if (typeof(q.settings.response)=="string") {
				this.response=q.settings.response;
			}
			this.render=function(o) {
				this.response=null;
				if (this.question.settings.response!==null && this.question.settings.response!='') this.response=this.question.settings.response;
				this.container=document.createElement("div");
...
//console.log(quizit.value.pages[quizit.page].questions[ans.question.question.answerIndex].answer.answers);
				...
				if (quizit.value.pages[quizit.page].questions[ans.question.question.answerIndex].answer.answers.length==quizit.value.pages[quizit.page].questions[ans.question.question.answerIndex].maximum) {
					var allCorrect=true;
					var answers=quizit.value.pages[quizit.page].questions[ans.question.question.answerIndex].answer.answers;
					for(var a=0;a<answers.length;a++) {
						//console.log(answers[a]);
						if (answers[a].score==0) allCorrect=false;	
					}
						//console.log(allCorrect);
					var question=quizit.pages[quizit.value.pages[quizit.page].key].questions[ans.question.question.answerIndex];
						//console.log(question);
					if (allCorrect && question.allresponse!==undefined && question.allresponse!==null) this.response=question.allresponse;
				}
				if (this.response==null && typeof(this.question.settings.response)=="string") {
					this.response=this.question.settings.response;
				}
		...
		draganddrop:function(q) {
			...
			this.render=function(o) {
				...
				$(this.container).find(".quizit_droparea").droppable({
					...
					drop:function(e,u) {
//console.log(u.helper.get(0).questionIndex,this.questionIndex);
		...
		reorder:function(q) {
			...
			this.render=function(o) {
				this.container=document.createElement("div");
				$(this.container).addClass("quizit_question").addClass("quizit_reorder").addClass("questionRequiresConfirmation").addClass("questionCompleted");
				$(this.container).html(this.title+this.paragraph);
			...
			this.answered=function() {
				var attemptCompletion=true;
				if (arguments.length>0) attemptCompletion=arguments[0];
				quizit.answeredQuestion=this.question;
				...
				for(var i=0;i<items.length;i++) {
					...
					if (i<this.question.settings.positions) {
						if (ans.index==i) {
							...
							if (this.response===null) this.response=this.options[items[i].index].response;
				...
				quizit.updateStatus();
				if (attemptCompletion && quizit.structure.pages[quizit.page].object.autoComplete()) quizit.completeQuestion();
				return true;
			};
		},
		text:function(q) {
			...
			this.render=function(o) {
				this.container=document.createElement("div");
				$(this.container).addClass("quizit_question").addClass("quizit_text").addClass("questionRequiresConfirmation");
				$(this.container).html(this.title+this.paragraph);
				var answers=document.createElement("div");
				...
				this.container.appendChild(answers);
				this.answered(ip.value,false);
				return this.container;
			};
			this.answered=function(ans) {
				var attemptCompletion=true;
				if (arguments.length>1) attemptCompletion=arguments[1];
				quizit.answeredQuestion=this.question;
				var q=this.question;
				this.response=null;
				...
				if (ans=="") {
					...
					for(var m=0;m<this.options.length;m++) {
						...
						var tt=new RegExp(toCheck);
						if (txt==toCheck || (this.question.settings.contains===true && tt.test(txt))) {
							quizit.value.pages[quizit.page].questions[q.answerIndex].answer.answers=[{index:m,value:ans,score:this.options[m].score}];

							this.response='';
							if (this.options[m].response!==undefined && this.options[m].response!==null && this.options[m].response.length>2) this.response=this.options[m].response;
						}
					}
				}
				if (this.response==null && typeof(q.settings.response)=="string") {
				...
				if (ans!="") {
					...
					$(this.container).removeClass("questionCompleted");
				}
				quizit.updateStatus();
				if (attemptCompletion && quizit.structure.pages[quizit.page].object.autoComplete()) quizit.completeQuestion();
				return true;
	...
	autoProgress:function() {
		...
		/*if (this.structure.pages[this.page].autoProgress===true) {
		...
		//console.log('autoProgress',this.structure.pages[this.page],this.canAutoProgress);
		if (this.structure.pages[this.page].autoProgress===true && this.canAutoProgress) {
			...
			for(var q=0;q<this.value.pages[this.page].questions.length;q++) {
				if (this.value.pages[this.page].questions[q].answer.answers.length===0) done=false;
			}
			if (this.structure.pages[this.page].serial===true && this.sequence<this.value.pages[this.page].questions.length) done=false;
			//console.log(this.sequence,this.value.pages[this.page].questions.length,done);
			if (done) {
				this.nextQuestion();
	...
	showResponses:function() {
		//this.structure.pages[this.page]
	},
		...
		for(var p=0;p<this.value.pages.length;p++) {
			for(var q=0;q<this.value.pages[p].questions.length;q++) {
				if (this.isNotEmpty(this.value.pages[p].questions[q].maximum)) {
					total+=parseInt(this.value.pages[p].questions[q].maximum);
				}else{
					total++;
				}
...
//console.log(this.value.pages[p].questions[q].answer);
					score+=parseInt(this.value.pages[p].questions[q].answer.answers[s].score);
				}
	...
	timer:{
		...
		drawTime:function() {
			...
			quizit.containers.timer.html("<span>Time left: <b>"+mins+":"+secs+"</b></span>");
	...
	completeQuestion:function() {
		...
		if (this.structure.pages[this.page].serial===true) {
			if (this.sequence==this.structure.pages[this.page].object.questions.length-1) {
				$('.yd_nextButton').html((this.page==this.structure.pages.length-1)?this.structure.finish:this.structure.nextPage).slideDown();
			}
//console.log(quizit.answeredQuestion);
//console.log('Complete question',this.page,this.sequence,this.structure.pages[this.page].object.questions[this.sequence]);
quizit.answeredQuestion.protect();
quizit.answeredQuestion.respond();
			...
			$('.yd_nextButton').unbind("click").stop().slideUp();
...
//console.log(this.structure.pages[this.page]);
		if (this.structure.pages[this.page].serial!==true) {
			...
			$($(this.containers.pages).find('.quizit_question').get(this.sequence)).stop().slideUp();
			this.sequence++;
			var qc=$(this.containers.pages).find('.quizit_question').get(this.sequence);
			
			$(qc).slideDown();
			if ($(qc).hasClass("questionRequiresConfirmation") || this.structure.pages[this.page].autoProgress===false){
				$('.yd_nextButton').stop().slideDown();
			}else{
				$('.yd_nextButton').stop().slideUp();
			}
			if (this.sequence<this.structure.pages[this.page].questions.length) {
				var q=this.structure.pages[this.page].questions[this.sequence];
				quizit.answeredQuestion=qc.source;
	...
	finished:function() {
		...
		if (this.value.completeOnPassOnly===true) this.completed=this.value.passed;
		this.value.score=this.status.percent.toFixed(0);
		this.value.time=duration;
		var txt='You scored {score}';
		var sound=null;
		if (this.structure.showResult!==false) {
			if (this.value.passed) {
				if (this.isNotEmpty(this.structure.passedMessage)) txt=this.structure.passedMessage;
				if (this.structure.passedSound!==undefined && this.structure.passedSound.url!="") sound=yoodoo.option.baseUrl+this.structure.passedSound.url;
			}else{
				if (this.isNotEmpty(this.structure.failedMessage)) txt=this.structure.failedMessage;
				if (this.structure.failedSound!==undefined && this.structure.failedSound.url!="") sound=yoodoo.option.baseUrl+this.structure.failedSound.url;
			}
		}else{
			if (this.isNotEmpty(this.structure.noResultMessage)) txt=this.structure.noResultMessage;
			if (this.structure.noResultSound!==undefined && this.structure.noResultSound.url!="") sound=yoodoo.option.baseUrl+this.structure.noResultSound.url;
		}
		if (!noSave) {
			if (this.value.passed) {
					if (this.isNotEmpty(this.structure.passTrigger) && this.structure.passTrigger>0) yoodoo.set_trigger({trigger:this.structure.passTrigger,dooit:yoodoo.lastLoad});
			}else{
					if (this.isNotEmpty(this.structure.failTrigger) && this.structure.failTrigger>0) yoodoo.set_trigger({trigger:this.structure.passTrigger,dooit:yoodoo.lastLoad});
			}
		}
		txt=txt.replace(/\{score\}/,"<b>"+this.value.score+"%</b>");
		this.containers.pages.html("<div class='quizit_complete "+((this.structure.showResult!==false)?(this.value.passed?"quizit_passed":"quizit_failed"):'')+"'>"+txt+"</div>");
