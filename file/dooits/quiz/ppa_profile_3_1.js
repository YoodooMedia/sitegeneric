...
var ppa_profile={
    ...
    processCompletion:function() {
        ...
        $(this.containers.area).animate({top:-10,opacity:0},this.animateDuration,function() {
            $(this).html("<div class='processing'>Fetching your results</div>").animate({top:0,opacity:1},ppa_profile.animateDuration,function() {
                ...
                yoodoo.sendPost(null,{context:ppa_profile,ppadata:Base64.encode(dooit.json(toSend)),cmd:'ppa',callback:'ppa_profile.gotResult'});
            });
        });
    },
	...
	var me=this;
        try{
            (yoodoo.ajax)? ppa=jQuery.parseJSON(result): ppa=jQuery.parseJSON(Base64.decode(result));
            //console.log(result);
           // (yoodoo.ajax)? eval('ppa='+result+';') : eval('ppa='+Base64.decode(result)+';');
            //console.log(ppa);
