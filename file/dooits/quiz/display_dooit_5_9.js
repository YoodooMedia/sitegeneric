...
//console.log(perform);
		if (perform) {
//console.log(this.value.randomized,groups);
...
//console.log(shuffled);
	...
	type_output:{
		...
		numeric:function(d,val,a) {
			...
			if (val.option.length==1) {
				for(var i=0;i<val.option.length;i++) ins+="<input type='text' style='text-align:right' onkeydown='return display_dooit.type_validate.numeric(event);' onkeyup='display_dooit.set_value(\""+d+"\","+i+",this.value,"+a+");' onblur='display_dooit.set_value(\""+d+"\","+i+",this.value,"+a+");' value='"+(display_dooit.value[d][a][i]?display_dooit.value[d][a][i]:"")+"' />";
			}else{
				for(var i=0;i<val.option.length;i++) ins+="<div class='item'><span class='label'>"+val.option[i].title+"</span><input type='text' onkeydown='return display_dooit.type_validate.numeric(event);' onkeyup='display_dooit.set_value(\""+d+"\","+i+",this.value,"+a+");' onblur='display_dooit.set_value(\""+d+"\","+i+",this.value,"+a+");' value='"+(display_dooit.value[d][a][i]?display_dooit.value[d][a][i]:"")+"' /></div>";
			}
			return ins;
		},
	...
	layout_buttons:function() {
		...
		for(var l=0;l<layouts.length;l++) {
			...
			var containerWidth=$(layouts[l]).width()-30;
...
//console.log(status.canProgress,this.questionDisplayed,(q.length-1));
	...
	canPDF:function() {
		return (typeof(summary)=="object" && yoodoo.toPDF!=undefined);
	},
	getPDF:function() {
		if (this.canPDF()) {
			this.pdfContent();
	...
	render:function() {
		...
		if (this.value.updated!==undefined) {
			...
			opa.push('<div style="float:right">'+inputs.formatDate('g:i a jS F Y',time)+'</div>');
		}
		...
		if (!this.data.dooit.serialize && !this.onlyReporting && this.report) op+="<button type='button' class='button toggle green' onclick='"+(this.reporting?'display_dooit.output();':'')+"display_dooit.toggle();'>"+(this.reporting?this.buttonText.edit:this.buttonText.report)+"</button>";
		if (this.canPDF()) op+="<button type='button' class='button toggle green' onclick='display_dooit.getPDF();'>get PDF</button>";
		if (this.reporting) {
		...
		if (this.reporting) {
			...
			//var h=$('#yoodooScrolledArea').height()-$(this.container).find('.result_report').offset().top;
			//setTimeout("$(this.container).find('.fields').css('height',"+h+");",1000);
			//$(this.container).find('.fields').css('overflow',"auto");
		}
	},
...
//console.log('Required',id,display_dooit.value[id],ok);
	...
	doTagging:function() {
		for(var i=0;i<this.data.structure.length;i++) {
			if (this.data.structure[i].value.type=="select" || this.data.structure[i].value.type=="checkbox" || this.data.structure[i].value.type=="radio" || this.data.structure[i].value.type=="buttons") {
