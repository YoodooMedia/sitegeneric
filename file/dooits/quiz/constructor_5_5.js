constructor_dooit={
	...
	help:{
		...
		randomize:{text:'Randomize will reorder the question groups on a user\'s first entry.<br /><em>Note: ungrouped paragraphs may end up out of order.</em>',width:300,left:false,top:true,xOffset:200,yOffset:0},
...
//((this.data.structure[d].paragraph=='')?'Undefined':sumText);
	...
	field_editor:function(d) {
		...
		ins+="<button type='button' style='display:"+(hideShuffle?'none':'block')+"' class='random"+((this.data.structure[d].value.randomize>=0)?' on':'')+"' onclick='constructor_dooit.groupRandomDialog(this);'>"+((this.data.structure[d].value.randomize>=0)?((this.data.structure[d].value.randomize>0)?'shuffle ['+this.data.structure[d].value.randomize+']':'shuffle all'):'unshuffled')+"</button>";
		...
		switch(this.data.structure[d].value.type) {
			case "paragraph":
				...
				op+="<div id='paragraph"+d+"' class='paragraph' onclick='constructor_dooit.edit_paragraph(this,"+d+")'>"+this.data.structure[d].paragraph+"</div>";
				...
				op+="<div id='reportparagraph"+d+"' style='margin:2px 0px' class='paragraph' onclick='constructor_dooit.edit_paragraph(this,"+d+",true)'>"+this.data.structure[d].reportparagraph+"</div>";
				op+="</div>";
				op+="<label><select onchange='constructor_dooit.set_paragraph_type(this,"+d+")'>";
	...
	tagOption:function(val) {
		...
		if (this.data.dooit.tag_rules!=undefined) {
			if (this.data.dooit.tag_rules.tags.length>0) {
				...
				ins+="<button type='button' class='default' onclick='constructor_dooit.tagOptionClicked(this)'>Add tag</button>";
	...
	tagOptionClicked:function(o) {
		...
		$(dropdown).find('#plus').css({
			...
			var value=$(constructor_dooit.parent_of_class(this,'fieldvalue')).prevAll('.fieldvalue').get().length;
			var field=$(constructor_dooit.parent_of_class(this,'field')).prevAll('.field').get().length;
			var tagName=$(this.parentNode).html().replace(/\<a.*a\>/gi,'').replace(/^ +/,'').replace(/ *$/,'');
			constructor_dooit.data.structure[field].value.option[value].tags.push([tagName,true]);
			this.parentNode.parentNode.parentNode.appendChild(constructor_dooit.tagOptionButton(tagName,true,true));
		...
		$(dropdown).find('#minus').css({
			...
			var field=$(constructor_dooit.parent_of_class(this,'field')).prevAll('.field').get().length;
			var tagName=$(this.parentNode).html().replace(/\<a.*a\>/gi,'').replace(/^ +/,'').replace(/ *$/,'');
			constructor_dooit.data.structure[field].value.option[value].tags.push([tagName,false]);
			this.parentNode.parentNode.parentNode.appendChild(constructor_dooit.tagOptionButton(tagName,false,true));
			$(this.parentNode.parentNode).remove();
		});
	...
	tagOptionButton:function(name,additive,asElement) {
		var ins=name+" <a href='javascript:void(0)' onclick='constructor_dooit.tagOptionRemove(this.parentNode)'>x</a>";
		if (asElement) {
	...
	tagOptionRemove:function(o) {
		...
		for(var i=constructor_dooit.data.structure[field].value.option[value].tags.length-1;i>=0;i--) {
			if (constructor_dooit.data.structure[field].value.option[value].tags[i][0]==tagName) constructor_dooit.data.structure[field].value.option[value].tags.splice(i,1);
		}
		$(o).remove();
	},
	type:{
...
//console.log(constructor_dooit.attachStructure[refs[ref][r]].value.option);
	...
	type_editor:{
		...
		textarea:function(val) {
			...
			if (val.length==0) op+="No input defined";
			for(var v=0;v<val.length;v++) {
				op+=constructor_dooit.type_option.textarea(val[v]);
			}
			op+="</div>";
			return op;
		},
		...
		radio:function(val) {
			var op="<a href='javascript:void(0)' onclick='constructor_dooit.add_option(this,\"radio\");'>add</a>";
			op+="<div class='fieldvalues'>";
			if (val.length==0) op+="No input defined";
			for(var v=0;v<val.length;v++) {
				op+=constructor_dooit.type_option.radio(val[v]);
			}
			op+="</div>";
			return op;
		},
	...
	type_option:{
		...
		text:function(val) {
			...
			ins+="<span style='position:relative'><a href='javascript:void(0)' onclick='constructor_dooit.textMatchDialog(this)'>"+val.matches.length+" Match"+((val.matches.length==1)?"":"es")+"</a></span>";
			ins+="<span style='position:relative'><a href='javascript:void(0)' onclick='constructor_dooit.textMetaDialog(this)'>"+val.meta.length+" Meta"+((val.meta.length==1)?"":"s")+"</a></span>";
		...
		textarea:function(val) {
			...
			var ins="<div class='move movevalue'></div><input type='text' value=\""+val.title.replace(/"/g,'&rdquo;')+"\" onkeyup='constructor_dooit.change(\"title\",this)' />";
			ins+="<span style='position:relative'><a href='javascript:void(0)' onclick='constructor_dooit.textMetaDialog(this)'>"+val.meta.length+" Meta"+((val.meta.length==1)?"":"s")+"</a></span>";
			ins+="<label><input type='checkbox' "+(val.fullwidth?"checked":"")+" onchange='constructor_dooit.change(\"fullwidth\",this)'/> full width</label>";
			var sel='';
			for(var r=1;r<20;r++) {
				sel+="<option value='"+r+"' "+((val.rows==r)?"selected":"")+">"+r+"</option>";
			}
			ins+="<label><select onchange='constructor_dooit.change(\"rows\",this)'>"+sel+"</select> rows</label>";
			ins+="<button class='removeButton' type='button' onclick='constructor_dooit.remove_option(this);'></button>";
			if (arguments.length>1 && arguments[1]) {
				var op=document.createElement("DIV");
				$(op).addClass("fieldvalue");
		...
		checkbox:function(val) {
			var ins="<div class='move movevalue'></div><input type='text' value=\""+val.title.replace(/"/g,'&rdquo;')+"\" onkeyup='constructor_dooit.change(\"title\",this)' />";
			if (constructor_dooit.score_system.display) {
				ins+=constructor_dooit.score_system.checkbox.selector(val.score,"");
				ins+=constructor_dooit.score_system.multiplier(val.multiplier);
			}
			ins+="<button class='default"+((val.isdefault=='1')?' on':'')+"' type='button' onclick='constructor_dooit.change(\"default\",this)'>default</button>";
			ins+=constructor_dooit.tagOption(val);
			ins+="<button class='removeButton' type='button' onclick='constructor_dooit.remove_option(this);'></button>";
			if (arguments.length>1 && arguments[1]) {
				var op=document.createElement("DIV");
				$(op).addClass("fieldvalue");
		...
		radio:function(val) {
			var ins="<div class='move movevalue'></div><input type='text' value=\""+val.title.replace(/"/g,'&rdquo;')+"\" onkeyup='constructor_dooit.change(\"title\",this)' />";
			if (constructor_dooit.score_system.display) {
				ins+=constructor_dooit.score_system.radio.selector(val.score,"");
				ins+=constructor_dooit.score_system.multiplier(val.multiplier);
			}
			ins+="<button class='default"+((val.isdefault=='1')?' on':'')+"' type='button' onclick='constructor_dooit.change(\"default\",this)'>default</button>";
			ins+=constructor_dooit.tagOption(val);
		...
		select:function(val) {
			var ins="<div class='move movevalue'></div><input type='text' value=\""+val.title.replace(/"/g,'&rdquo;')+"\"onkeyup='constructor_dooit.change(\"title\",this)' />";
			if (constructor_dooit.score_system.display) {
				ins+=constructor_dooit.score_system.select.selector(val.score,"");
				ins+=constructor_dooit.score_system.multiplier(val.multiplier);
			}
			ins+="<button class='default"+((val.isdefault=='1')?' on':'')+"' type='button' onclick='constructor_dooit.change(\"default\",this)'>default</button>";
			ins+=constructor_dooit.tagOption(val);
		...
		buttons:function(val) {
			var ins="<div class='move movevalue'></div><input type='text' value=\""+val.title.replace(/"/g,'&rdquo;')+"\" onkeyup='constructor_dooit.change(\"title\",this)' />";
			if (constructor_dooit.score_system.display) {
				ins+=constructor_dooit.score_system.buttons.selector(val.score,"");
				ins+=constructor_dooit.score_system.multiplier(val.multiplier);
			}
			ins+="<button class='default"+((val.isdefault=='1')?' on':'')+"' type='button' onclick='constructor_dooit.change(\"default\",this)'>default</button>";
			ins+=constructor_dooit.tagOption(val);
	...
	groupRandomDialog:function(o) {
		...
		if (constructor_dooit.data.structure[this.groupRandomEdit.index].value.randomize==undefined) constructor_dooit.data.structure[this.groupRandomEdit.index].value.randomize=-1;
		var c=1;
		var i=this.groupRandomEdit.index+1;
		while(i<constructor_dooit.data.structure.length && constructor_dooit.data.structure[i].value.grouped=="1") {
			i++;
			c++;
		}
		...
		for(var k in opts) {
			...
			$(opt).bind("click",function() {
				constructor_dooit.change('randomize',constructor_dooit.groupRandomEdit.button,constructor_dooit.groupRandomEdit.index,this.value);
				constructor_dooit.dropRandomDialog();
			});
	...
	textMetaDialog:function(o) {
		...
		$(addButton).bind("click",function() {
			constructor_dooit.data.structure[constructor_dooit.textMetaOptions.fieldIndex].value.option[constructor_dooit.textMetaOptions.optionIndex].meta.push({text:''});
			var matches=constructor_dooit.data.structure[constructor_dooit.textMetaOptions.fieldIndex].value.option[constructor_dooit.textMetaOptions.optionIndex].meta;
			constructor_dooit.textMetaOptions.dialog.insertBefore(constructor_dooit.textMetaEntry(matches[matches.length-1]),this);
			constructor_dooit.textMetaUpdateButton();
		}).addClass("default");
		addButton.innerHTML="add";
		dialog.appendChild(addButton);
		var closeButton=document.createElement("button");
		$(closeButton).bind("click",function() {constructor_dooit.dropTextMetaDialog();}).addClass("default").html("close");
	...
	textMatchDialog:function(o) {
		...
		var addButton=document.createElement("button");
		$(addButton).bind("click",function() {
			constructor_dooit.data.structure[constructor_dooit.textMatchOptions.fieldIndex].value.option[constructor_dooit.textMatchOptions.optionIndex].matches.push({text:'',score:null});
			var matches=constructor_dooit.data.structure[constructor_dooit.textMatchOptions.fieldIndex].value.option[constructor_dooit.textMatchOptions.optionIndex].matches;
			constructor_dooit.textMatchOptions.dialog.insertBefore(constructor_dooit.textMatchEntry(matches[matches.length-1]),this);
			constructor_dooit.textMatchUpdateButton();
		}).addClass("default");
		addButton.innerHTML="add";
		dialog.appendChild(addButton);
		var closeButton=document.createElement("button");
		$(closeButton).bind("click",function() {constructor_dooit.dropTextMatchDialog();}).addClass("default").html("close");
		dialog.appendChild(closeButton);
		$(dialog).css({position:"absolute",top:0,right:0});
		$(dialog).addClass("textMatch");
		o.parentNode.appendChild(dialog);
	...
	textMetaUpdateButton:function() {
		var l=constructor_dooit.data.structure[constructor_dooit.textMetaOptions.fieldIndex].value.option[constructor_dooit.textMetaOptions.optionIndex].meta.length;
		$(constructor_dooit.textMetaOptions.dialog.parentNode).find(">a").html(l+' Meta'+((l==1)?'':'s'));
	},
	textMatchUpdateButton:function() {
		var l=constructor_dooit.data.structure[constructor_dooit.textMatchOptions.fieldIndex].value.option[constructor_dooit.textMatchOptions.optionIndex].matches.length;
		$(constructor_dooit.textMatchOptions.dialog.parentNode).find(">a").html(l+' Match'+((l==1)?'':'es'));
	},
	textMetaEntry:function(matchItem) {
		...
		$(ip).bind("keyup",function() {
			...
			constructor_dooit.data.structure[constructor_dooit.textMetaOptions.fieldIndex].value.option[constructor_dooit.textMetaOptions.optionIndex].meta[mi].text=this.value;
		});
		...
		$(rem).bind("click",function() {
			var mi=$(this.parentNode).prevAll("div").get().length;
			constructor_dooit.data.structure[constructor_dooit.textMetaOptions.fieldIndex].value.option[constructor_dooit.textMetaOptions.optionIndex].meta.splice(mi,1);
	...
	textMatchEntry:function(matchItem) {
		...
		$(ip).bind("keyup",function() {
			var mi=$(this.parentNode).prevAll("div").get().length;
			constructor_dooit.data.structure[constructor_dooit.textMatchOptions.fieldIndex].value.option[constructor_dooit.textMatchOptions.optionIndex].matches[mi].text=this.value;
		});
		ip.value=matchItem.text;
		if(matchText.childNodes.length>0) {
			matchText.insertBefore(ip,matchText.childNodes[0]);
		...
		$(rem).bind("click",function() {
			var mi=$(this.parentNode).prevAll("div").get().length;
			constructor_dooit.data.structure[constructor_dooit.textMatchOptions.fieldIndex].value.option[constructor_dooit.textMatchOptions.optionIndex].matches.splice(mi,1);
			$(this.parentNode).remove();
			constructor_dooit.textMatchUpdateButton();
		}).addClass("removeButton");
		matchText.appendChild(rem);
	...
	setTextMatchScore:function(o) {
		...
		constructor_dooit.data.structure[constructor_dooit.textMatchOptions.fieldIndex].value.option[constructor_dooit.textMatchOptions.optionIndex].matches[mi].score=o.value;
	},
	setMultiplier:function(o,v) {
		...
		var fv=this.parent_of_class(o,"fieldvalue");
		var fvi=$(fv).prevAll('.fieldvalue').get().length;
		if (constructor_dooit.data.structure[fi].value.type=="text") {
			var mi=$(o.parentNode.parentNode).prevAll("div").get().length;
			if (constructor_dooit.data.structure[fi].value.option[fvi].matches[mi].multiplier==undefined || isNaN(constructor_dooit.data.structure[fi].value.option[fvi].matches[mi].multiplier)) constructor_dooit.data.structure[fi].value.option[fvi].matches[mi].multiplier=1;
			constructor_dooit.data.structure[fi].value.option[fvi].matches[mi].multiplier+=v;
			if (constructor_dooit.data.structure[fi].value.option[fvi].matches[mi].multiplier<1) constructor_dooit.data.structure[fi].value.option[fvi].matches[mi].multiplier=1;
			$(o.parentNode).find("span").html(constructor_dooit.data.structure[fi].value.option[fvi].matches[mi].multiplier);
		}else{
			if (constructor_dooit.data.structure[fi].value.option[fvi].multiplier==undefined || isNaN(constructor_dooit.data.structure[fi].value.option[fvi].multiplier)) constructor_dooit.data.structure[fi].value.option[fvi].multiplier=1;
			constructor_dooit.data.structure[fi].value.option[fvi].multiplier+=v;
			if(constructor_dooit.data.structure[fi].value.option[fvi].multiplier<1) constructor_dooit.data.structure[fi].value.option[fvi].multiplier=1;
			$(o.parentNode).find("span").html(constructor_dooit.data.structure[fi].value.option[fvi].multiplier);
		}
	},
	change:function(k,o) {
		if(k=="start") {
...
//console.log(o.parentNode.parentNode.parentNode);
			if(checked) {
				...
				if(i>0) {
					if (constructor_dooit.data.structure[i-1].value.grouped!="1") prevRandom.css({display:'block'});
				}
			...
			constructor_dooit.data.structure[i].value.randomize=v;
		}else if(k=="default") {
			var checked=!$(o).hasClass("on");
			var i=$(o.parentNode.parentNode.parentNode.parentNode.parentNode).prevAll('.field').get().length;
			var j=$(o.parentNode).prevAll('.fieldvalue').get().length;
...
//console.log(this.fields_content);
	...
	edit_paragraph:function(o,d) {
		...
		libraryButton = nicEditorAdvancedButton.extend({   
		  mouseClick : function() {
			constructor_dooit.htmlInstance.saveRng();
			  constructor_dooit.uploader();
		  }
		});
		...
		if(this.nic===null) this.nic=new nicEditor({fullPanel:false,buttonList:bl,iconsPath:yoodoo.getFilePath('image',true)+'dooits/nicEditorIcons.gif'});
...
//console.log(this.data.structure[this.open_editor.question]);
	...
	uploaderWindow:null,
	uploader:function() {
		var res=constructor_dooit.iframeResponse;
		...
		var ins='<iframe src="'+yoodoo.option.baseUrl+'imagery/uploader/?repository=dooits&quantity=0&sitehash='+sitehash+'&userhash='+userhash+'&width='+w+'&height='+h+'&callback=constructor_dooit.imageSelected&origin='+window.location.hostname+'" style="width:'+w+'px;height:'+h+'px" scrolling="no" ></iframe>';
	...
	uploaderClosed:function() {
		if (window.detachEvent) {
			window.detachEvent('onmessage',constructor_dooit.iframeResponse);
		}else{
			window.removeEventListener('message',constructor_dooit.iframeResponse,false);
		}
	},
	...
	imageSelected:function(img) {
		constructor_dooit.htmlInstance.restoreRng();
		constructor_dooit.nic.nicCommand("insertImage",yoodoo.option.baseUrl+img.source.replace(/^\//,''));
		var imgs=$(constructor_dooit.htmlInstance.elm).find("IMG").get();
		for(var i=0;i<imgs.length;i++) {
			if(imgs[i].src==img.source) imgs[i].title=img.title;
		}
		$(constructor_dooit.uploaderWindow).dialog("close");
	},
	...
	tag_rule_editor:{
		...
		init:function() {
			...
			if(constructor_dooit.score_system.range.length>0) {
				...
				this.build_set();
				$(this.container).addClass("tag_rule_editor");
				$(this.container).html('<a href="javascript:void(0)" onclick="constructor_dooit.tag_rule_editor.toggle(this)">Show Tag Rule editor</a><div style="display:none"><div class="tab_content"><div class="tag_content"></div><div class="rule_content"></div></div></div>');
				$(this.container).find('.tag_content').get(0).appendChild(this.tag_list());
			}
		...
		rule_item:function(r) {
			...
			if (constructor_dooit.data.dooit.tag_rules.rules[r].unset==undefined) constructor_dooit.data.dooit.tag_rules.rules[r].unset=false;
			ins+='<label><span>Unset</span>: <input type="checkbox" '+(constructor_dooit.data.dooit.tag_rules.rules[r].unset?' checked':'')+' onchange="constructor_dooit.tag_rule_editor.update_rule(\'unset\',this)"/></label>';
			...
			ins+="<button type='button' onclick='constructor_dooit.tag_rule_editor.remove_rule(this)' class='removeButton'></button><br />";
			...
			ins+='<label>Lowest: <input type="checkbox" '+(constructor_dooit.data.dooit.tag_rules.rules[r].lowest?' checked':'')+' onchange="constructor_dooit.tag_rule_editor.update_rule(\'lowest\',this)"/></label><br />';


			ins+='<label>Compare: <input type="checkbox" '+(constructor_dooit.data.dooit.tag_rules.rules[r].compare?' checked':'')+' onchange="constructor_dooit.tag_rule_editor.update_rule(\'compare\',this);$(this.parentNode).next().css(\'display\',this.checked?\'inline\':\'none\');"/></label>';
			ins+='<span style="display:'+(constructor_dooit.data.dooit.tag_rules.rules[r].compare?' inline':'none')+'">';

			ins+='<label>by: <select onchange="constructor_dooit.tag_rule_editor.update_rule(\'compareby\',this)" >';
			var types={none:'',lt:'&lt;',lte:'&lt;=',eq:'=',gte:'&gt;=',gt:'&gt;'};
			for(var i in types) {
				ins+="<option value='"+i+"' "+((constructor_dooit.data.dooit.tag_rules.rules[r].compareby==i)?' selected="true"':'')+">"+types[i]+"</option>";
			}
			ins+="</select></label>";

			ins+='<label>with: <select onchange="constructor_dooit.tag_rule_editor.update_rule(\'comparewith\',this)" >';
			ins+="<option></option>";
			for(var i=0;i<constructor_dooit.score_system.range.length;i++) {
				ins+="<option"+((constructor_dooit.data.dooit.tag_rules.rules[r].comparewith==constructor_dooit.score_system.range[i])?' selected="true"':'')+">"+constructor_dooit.score_system.range[i]+"</option>";
			}
			ins+='</select></label>';

			ins+="</span><br />";
			if(constructor_dooit.data.dooit.tag_rules.rules[r].above==undefined) constructor_dooit.data.dooit.tag_rules.rules[r].above=false;
			if(constructor_dooit.data.dooit.tag_rules.rules[r].below==undefined) constructor_dooit.data.dooit.tag_rules.rules[r].below=false;
			if(constructor_dooit.data.dooit.tag_rules.rules[r].abovePercent==undefined) {
				constructor_dooit.data.dooit.tag_rules.rules[r].abovePercent=50;
				constructor_dooit.data.dooit.tag_rules.rules[r].above=false;
			}
			if(constructor_dooit.data.dooit.tag_rules.rules[r].belowPercent==undefined) {
				constructor_dooit.data.dooit.tag_rules.rules[r].belowPercent=50;
				constructor_dooit.data.dooit.tag_rules.rules[r].below=false;
			}
			ins+='<span>Above: <input type="checkbox" '+(constructor_dooit.data.dooit.tag_rules.rules[r].above?' checked':'')+' onchange="constructor_dooit.tag_rule_editor.update_rule(\'above\',this)"/> <input type="text" onchange="constructor_dooit.tag_rule_editor.update_rule(\'abovePercent\',this)" value="'+constructor_dooit.data.dooit.tag_rules.rules[r].abovePercent+'" style="width:30px;display:'+(constructor_dooit.data.dooit.tag_rules.rules[r].above?'inline':'none')+'"  />%</span>';
			ins+='&nbsp;&nbsp;<span>Below: <input type="checkbox" '+(constructor_dooit.data.dooit.tag_rules.rules[r].below?' checked':'')+' onchange="constructor_dooit.tag_rule_editor.update_rule(\'below\',this)"/> <input type="text" onchange="constructor_dooit.tag_rule_editor.update_rule(\'belowPercent\',this)" value="'+constructor_dooit.data.dooit.tag_rules.rules[r].belowPercent+'" style="width:30px;display:'+(constructor_dooit.data.dooit.tag_rules.rules[r].below?'inline':'none')+'" />%</span>';
			$(rule).html(ins);
			return rule;
		},
		update_rule:function(t,o) {
			var i=$(constructor_dooit.parent_of_class(o,'rule_item')).prevAll('.rule_item').get().length;
			if (t=="highest" || t=="lowest" || t=="unique" || t=="compare" || t=="above" || t=="below" || t=="unset") {
				...
				if(v<0) v=0;
				if(v>100) v=0;
				constructor_dooit.data.dooit.tag_rules.rules[i][t]=v;
			}else{
	...
	scoring:{
		...
		rag_balloons:{
			...
			multiplier:function(v) {
				...
				var ins="<nobr> x<span>"+v+"</span><button class='default' onclick='constructor_dooit.setMultiplier(this,-1)'>-</button><button class='default' onclick='constructor_dooit.setMultiplier(this,1)'>+</button></nobr>";
				return ins;
			},
			checkbox:{
			...
			textMatch:{
				selector:function(val,attrs) {
					var ins="<select onchange='constructor_dooit.setTextMatchScore(this);"+attrs+"'>";
					ins+="<option value='none'>none</option>";
					for(var k=0;k<constructor_dooit.score_system.range.length;k++) {
						var s=constructor_dooit.score_system.range[k];
						ins+="<option value='"+s+"'"+((s==val)?" selected='selected'":"")+">"+s+"</option>";
		...
		multi_balloons:{
			...
			optionsDisplay:function(){
				...
				ins+="</select></div>";
				ins+='<div class="info" title="More than 6 will not display anyway"><label>Display? </label><input type="checkbox" '+(constructor_dooit.score_system.options.display?'checked':'')+' onchange="constructor_dooit.score_system.options.display=!constructor_dooit.score_system.options.display;constructor_dooit.score_system.saveOptions();"/>';
				ins+="</select></div>";
				return ins;
			},
			multiplier:function(v) {
