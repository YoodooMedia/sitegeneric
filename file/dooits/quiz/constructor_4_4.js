...
//'"'+o.replace(/"/g,'\\"')+'"';
	...
	tag_rule_editor:{
		...
		rule_item:function(r) {
			...
			ins+='<label>Above: <input type="checkbox" '+(constructor_dooit.data.dooit.tag_rules.rules[r].above?' checked':'')+' onchange="constructor_dooit.tag_rule_editor.update_rule(\'above\',this);$(this.parentNode).next().css(\'display\',this.checked?\'inline\':\'none\');"/></label>';
			ins+='<label style="display:'+(constructor_dooit.data.dooit.tag_rules.rules[r].above?' inline':'none')+'">Above colour: <select onchange="constructor_dooit.tag_rule_editor.update_rule(\'abovecolour\',this)" >';
			ins+="<option></option>";
			for(var i=0;i<constructor_dooit.score_system.range.length;i++) {
				ins+="<option"+((constructor_dooit.data.dooit.tag_rules.rules[r].abovecolour==constructor_dooit.score_system.range[i])?' selected="true"':'')+">"+constructor_dooit.score_system.range[i]+"</option>";
			}
			ins+="</select></label>";

			ins+='<label>Below: <input type="checkbox" '+(constructor_dooit.data.dooit.tag_rules.rules[r].below?' checked':'')+' onchange="constructor_dooit.tag_rule_editor.update_rule(\'below\',this);$(this.parentNode).next().css(\'display\',this.checked?\'inline\':\'none\');"/></label>';
			ins+='<label style="display:'+(constructor_dooit.data.dooit.tag_rules.rules[r].below?' inline':'none')+'">Below colour: <select onchange="constructor_dooit.tag_rule_editor.update_rule(\'belowcolour\',this)" >';
			ins+="<option></option>";
			for(var i=0;i<constructor_dooit.score_system.range.length;i++) {
				ins+="<option"+((constructor_dooit.data.dooit.tag_rules.rules[r].belowcolour==constructor_dooit.score_system.range[i])?' selected="true"':'')+">"+constructor_dooit.score_system.range[i]+"</option>";
			}
			ins+="</select></label>";
		...
		update_rule:function(t,o) {
			...
			if (t=="highest" || t=="lowest" || t=="unique" || t=="above" || t=="below") {
