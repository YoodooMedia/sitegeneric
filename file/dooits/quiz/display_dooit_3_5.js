...
// attach any reference structure/date
		...
		dooit.setExport({csv:display_dooit.xport,xls:display_dooit.xportxls});
		if(this.data.dooit.serialize) {
	...
	set_button:function(o) {
		...
		display_dooit.value[ans.id][ans.answer][si]='1';
	...
	value_updated:function() {
		...
		if(this.data.dooit.serialize) {
			...
			if(typeof(this.completed[this.data.structure[this.questionDisplayed].value.type])!="undefined") {
				if(!this.completed[this.data.structure[this.questionDisplayed].value.type](i)) canProgress=false;
			}
			if(this.auto_move[this.data.structure[i].value.type]) {
	...
	type_report:{
		...
		text:function(d,val,a) {
			var ins="";
			if (val.length>1) {
				for(var i=0;i<val.length;i++) ins+="<div class='item'>"+val[i].title+" <span class='ans'>"+(display_dooit.value[d][a][i]?display_dooit.value[d][a][i]:"")+"</span></div>";
...
//if(forPrinter) return "For Printer";
	...
	field_report_answer:function(d,a) {
		...
		if (!forPrinter) {
			...
			ins+="<p class='label'>"+this.data.structure[d].title+"</p>";
			ins+="<p class='answer'>"+this.type_report[this.data.structure[d].value.type](this.data.structure[d].id,this.data.structure[d].value.option,a)+"</p>";
		}
		if(asElement) {
			var e=document.createElement("DIV");
			$(e).addClass('answer_item');
