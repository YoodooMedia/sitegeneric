...
//console.log(status.canProgress,this.questionDisplayed,(q.length-1));
	...
	render:function() {
		...
		if (this.value.updated!==undefined) {
			...
			opa.push('<div style="float:right">Completed: '+inputs.formatDate('g:i a jS F Y',time)+'</div>');
		}
	...
	setFieldsSize:function() {
		if (typeof(display_dooit.data.dooit.score_options)=="undefined") return '';
		if (typeof(display_dooit.data.dooit.score_options)=="undefined") return '';
		if (display_dooit.data.dooit.score_options!==null && display_dooit.data.dooit.score_options.display===false) {
			var h=$('#yoodooScrolledArea').height()-$(display_dooit.container).find('.result_report').offset().top;
			$(display_dooit.container).find('.fields').css('height',h);
			$(display_dooit.container).find('.fields').css('overflow',"auto");
		}
	},
