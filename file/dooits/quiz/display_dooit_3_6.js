	...
	type_output:{
		...
		text:function(d,val,a) {
			...
			if (val.option.length==0) {
				...
				for(var i=0;i<val.option.length;i++) ins+="<div class='item2'><span class='label'>"+val.option[i].title+"</span><input type='text' onkeyup='display_dooit.set_value(\""+d+"\","+i+",this.value,"+a+");' value='"+(display_dooit.value[d][a][i]?display_dooit.value[d][a][i]:"")+"' /></div>";
			}
			return ins;
		},
		numeric:function(d,val,a) {
			...
			if (val.option.length==1) {
				...
				for(var i=0;i<val.option.length;i++) ins+="<div class='item2'><span class='label'>"+val.option[i].title+"</span><input type='text' onkeydown='return display_dooit.type_validate.numeric(event);' onkeyup='display_dooit.set_value(\""+d+"\","+i+",this.value,"+a+");' value='"+(display_dooit.value[d][a][i]?display_dooit.value[d][a][i]:"")+"' /></div>";
			}
			return ins;
		},
		checkbox:function(d,val,a) {
			...
			if (val.option.length==1) {
				...
				for(var i=0;i<val.option.length;i++) ins+="<div class='item2'><label><span class='label'>"+val.option[i].title+"</span><input type='checkbox' "+((display_dooit.value[d][a][i]==1)?"checked":"")+" onchange='display_dooit.set_value(\""+d+"\","+i+",this.checked?\"1\":\"0\","+a+");'/></label></div>";
			}
			return ins;
		},
...
//if(forPrinter) return "For Printer";
	...
	field_answer:function(d,a) {
		...
		if ((!hideFirstTitle && head) || this.data.dooit.serialize) ins+="<div class='label'>"+tit+"</div>";
		...
		if (this.data.structure[d].value.option.length>0) {
			...
			if (!this.data.dooit.serialize && addItemDiv && !hideFirstTitle && this.data.structure[d].title!='') {
