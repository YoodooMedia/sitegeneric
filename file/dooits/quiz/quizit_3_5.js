...
//console.log(this.nextResponse);
		...
		this.protect=function() {
			...
			$(this.hide).css({position:"absolute",width:'100%',height:'100%',padding:0,top:0,left:0,background:'rgba(255,255,255,0.9)'});
...
//console.log(this.structure.pages[this.page]);
	...
	loadFields:function() {
		if (typeof(array_of_default_fields)=="object" && array_of_default_fields.length==2
			...
			if (this.key!="") {
				for(var k=0;k<array_of_default_fields.length;k++) {
					if (array_of_default_fields[k]!=this.construct_key) this.valuekey=array_of_default_fields[k];
				}
			}
