...
var summary={
	...
	prime:function() {
		if (yoodoo.defaults!==undefined) {
			for(var type in this.styles) {
				if (yoodoo.defaults['pdf'+type]!==undefined) this.styles[type]=yoodoo.defaults['pdf'+type];
			}
		}
	},
	build:function() {
	...
	//console.log(this.fields[f]);
		...
		if (this.displayOutput) {
			$(this.container).html("<h2>"+yoodoo.dooittitle+"</h2><button type='button' class='printButton'></button><div class='summaryPrinter'><div id='summaryPreview'></div></div><div class='summarySelector'></div>");
			this.selector();
		}
		summary.display();
						...
						case "elevatorPitch":
							...
							if(this.values[this.printable[p][2]].pitches!=undefined && this.values[this.printable[p][2]].pitches.length>0) {
								...
								for(var pp=0;pp<pitches.length;pp++) {
									...
									if (pitches.length>1) summary.printJSON.push(summary.printH2(pitches[pp].name));
									summary.printJSON.push(summary.printLabel(pitches[pp].content));
									if (pitches.length>1) ins+="<h3>"+pitches[pp].name+"</h3>";
									ins+="<p>"+pitches[pp].content+"</p>";
								}
							}
							txt+=ins;
...
//console.log(this.values[this.printable[p][2]].customer.length);
	...
	sendPdf:function() {
		if (yoodoo.defaults!==undefined && yoodoo.defaults.pdfHeader!==undefined) summary.pdfHeader=yoodoo.defaults.pdfHeader;
		...
		yoodoo.toPDF({header:summary.pdfHeader,content:summary.printJSON,filename:yoodoo.dooittitle.replace(/[^a-z^0-9]+/ig,'')});
	},
	print:function() {
