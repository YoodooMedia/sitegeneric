	...
	analyze:function() {
		...
		if(display_dooit.value.traits.invalid) {
			...
			for(var i=0;i<display_dooit.value.traits.readOrder.length;i++) {
				...
				var col=[];
				for(var c=0;c<charlist[display_dooit.value.traits.readOrder[i]][idx].length;c++) {
					if (this.verboseCharacteristics) {
						col.push(charlist[display_dooit.value.traits.readOrder[i]][idx][c]);
					}else{
						col.push(charlist[display_dooit.value.traits.readOrder[i]][idx][c].toLowerCase());
					}
				}
