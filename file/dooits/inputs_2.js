...
inputs={
	...
	dropdown:function(objs) {
		...
		for(var oo=0;oo<objs.length;oo++) {
			...
			objs[oo].physicalElement=selectBox;
...
document.body.appendChild(o.displaying);
					...
					o.displaying.drawHead=function() {
						...
						if (this.source.opts.past || (!this.source.opts.past && !(new Date(this.date.getFullYear(),this.date.getMonth(),-1)<new Date()))) {
							if ((this.source.opts.minDate===null) || (new Date(this.date.getFullYear(),this.date.getMonth(),-1)>=this.source.opts.minDate)) {
							...
							if ((this.source.opts.minDate===null) || (new Date(this.date.getFullYear()-1,this.date.getMonth(),1)>=this.source.opts.minDate)) {
								var el=document.createElement("BUTTON");
								$(el).addClass("left");
								$(el).html(this.source.opts.leftText+this.source.opts.leftText);
								el.source=this;
								$(el).bind("click",function(e) {
									// go back a month
									e.preventDefault();
									var o=this;
									o.source.date=new Date(o.source.date.getFullYear()-1,o.source.date.getMonth(),o.source.date.getDate());
									var tarTD=null;
									if (o.source.source.opts.slide) {
										tarTD=$(o.source).find('.c .monthCell').get(0);
									}else{
						...
						if ((this.source.opts.maxDate===null) || (new Date(this.date.getFullYear(),this.date.getMonth()+1,1)<=this.source.opts.maxDate)) {
							var el=document.createElement("BUTTON");
							$(el).addClass("right");
						...
						if ((this.source.opts.maxDate===null) || (new Date(this.date.getFullYear()+1,this.date.getMonth(),1)<=this.source.opts.maxDate)) {
							var el=document.createElement("BUTTON");
							$(el).addClass("right");
							$(el).html(this.source.opts.rightText+this.source.opts.rightText);
							el.source=this;
							$(el).bind("click",function(e) {
								// go forward a month
								var o=this;
					...
					o.position();
					o.blockout.source=o;
					$(o.blockout).bind("click",function(e) {
						var o=this;
			...
			objs[oo].position=function() {
				...
				var maxBottom=$(window).height()+window.scrollY;
		...
		function monthTable(dt,opts) {
			...
			for(var d=0;d<days.length;d++) {
				op+="<td>"+days[d].substring(0,opts.daynamelength)+"</td>";
			};
			op+="</tr>";
	...
	formatDate:function(f,dt) {
		if (typeof(yoodoo)!="undefined" && yoodoo.formatDate!==undefined) return yoodoo.formatDate(f,dt);
		return dt.toString();
	},
