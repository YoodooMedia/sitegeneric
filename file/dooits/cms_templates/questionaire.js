questionaire={
	...
	init:function() {
		...
		$('form#exercise_form').submit(function() {
			questionaire.save();
		});
...
//console.log(questionaire.attachStructure[refs[ref][r]].value.option);
	...
	textMatchDialog:function(o) {
		...
		dialog.appendChild(addButton);
		var closeButton=document.createElement("button");
		$(closeButton).bind("click",function() {questionaire.dropTextMatchDialog();}).addClass("default").html("close");
		dialog.appendChild(closeButton);
		$(dialog).css({position:"absolute",top:0,right:0});
		$(dialog).addClass("textMatch");
		o.parentNode.appendChild(dialog);
	...
	scoring:{
		...
		rag_balloons:{
			...
			multiplier:function(v) {
				...
				var ins="<nobr> x<span>"+v+"</span><button class='default' onclick='questionaire.setMultiplier(this,-1)'>-</button><button class='default' onclick='questionaire.setMultiplier(this,1)'>+</button></nobr>";
				return ins;
			},
			checkbox:{
