...
//((this.data.structure[d].paragraph=='')?'Undefined':sumText);
	...
	hasWidget:function(val) {
		for(var o=0;o<val.option.length;o++) {
			if (val.option[o].widget!==undefined && val.option[o].widget!==null && val.option[o].widget.exerciseId>0) return true;
		}
		return false;
	},
	field_editor:function(d) {
		...
		switch(this.data.structure[d].value.type) {
			...
			default:
				...
				var showPara=(head || this.data.structure[d].value.grouped!="1");
				op+="<div class='paraDisplay' style='display:"+(showPara?'block':'none')+"'>"+this.paragraph_title+"<div style='clear:both'><div id='paragraph"+d+"' class='paragraph head' onclick='questionaire.edit_paragraph(this,"+d+")'>"+((typeof(this.data.structure[d].paragraph)!="undefined")?this.data.structure[d].paragraph:'')+"</div></div></div>";
				op+="Title: <input type='text' onkeydown='return questionaire.titleKeys(event)' value=\""+this.data.structure[d].title.replace(/"/g,'&rdquo;')+"\" onkeyup='questionaire.edit_title(this);' />"+this.type_selector(this.data.structure[d].value.type)+this.type_help[this.data.structure[d].value.type]()+"</div>";
				var hw=questionaire.hasWidget(this.data.structure[d].value);
...
//console.log(i,j,k);
	...
	render:function() {
		...
		op+="<div class='info helper'>"+this.helpMessage('serialize')+"<label>Serialize:</label><input type='checkbox' "+((typeof(this.data.dooit.serialize)=="undefined")?"":(this.data.dooit.serialize?"checked":""))+" onchange='questionaire.data.dooit.serialize=this.checked;$(this.parentNode).next().css(\"display\",this.checked?\"block\":\"none\");' /></div>";
...
//'"'+o.replace(/"/g,'\\"')+'"';
	...
	tag_rule_editor:{
		...
		rule_item:function(r) {
			...
			ins+='<label>Lowest: <input type="checkbox" '+(questionaire.data.dooit.tag_rules.rules[r].lowest?' checked':'')+' onchange="questionaire.tag_rule_editor.update_rule(\'lowest\',this)"/></label><br />';
			...
			if(questionaire.data.dooit.tag_rules.rules[r].abovePercent==undefined || questionaire.data.dooit.tag_rules.rules[r].abovePercent=='') {
				questionaire.data.dooit.tag_rules.rules[r].abovePercent=0;
				questionaire.data.dooit.tag_rules.rules[r].above=false;
			}
			if(questionaire.data.dooit.tag_rules.rules[r].belowPercent==undefined ||questionaire.data.dooit.tag_rules.rules[r].belowPercent=='') {
				questionaire.data.dooit.tag_rules.rules[r].belowPercent=100;
...
/*	Scoring systems	*/
	...
	scoring:{
		...
		galileo:{
			...
			multiplier:function(v) {
				if (v==undefined || isNaN(v) || v<1) v=1;
				var ins="<nobr> x<span>"+v+"</span><button type='button' class='default' onclick='questionaire.setMultiplier(this,-1)'>-</button><button class='default' onclick='questionaire.setMultiplier(this,1)'>+</button></nobr>";
				return ins;
			},
			checkbox:{
				selector:function(val,attrs) {
