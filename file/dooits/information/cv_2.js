...
var cv={
	...
	init:function(personalKey,profileKey,careerKey,educationKey,hobbyKey,refereesKey,cvKey) {
		...
		this.container=$('.dooitDisplay.cv').get(0);
	...
	cv_parameters:function() {
		...
		var wordButton=this.createElement("button",null,'layoutButton',{margin:'0 0 2px 20px'});
		$(wordButton).attr("type","button");
		$(wordButton).html("as Word doc");
		$(wordButton).bind("click",function() {cv.msdoc();});
		row1.appendChild(this.cv_inputs.title);
		row1.appendChild(prevButton);
		row1.appendChild(wordButton);
	...
	msdocFrame:null,
	msdoc:function(){
		if (this.msdocFrame!==null) $(this.msdocFrame).remove();
		var html=this.outputHTML();
		var docCreateURL='http://www.yoodoo.biz/utilities/msdoc.php';
		var op='';
		op+="<html><head></head><body><form id='yoodooPost' action='"+docCreateURL+"' method='POST'>";
		op+="<textarea name='msdoc'>"+html+"</textarea>";
		op+="<textarea type='text' name='style'>"+this.cv.array[this.showing].style+"</textarea>";
		op+="<textarea type='text' name='name'>"+this.cv.array[this.showing].name+"</textarea>";
		op+="</form>";
		op+="<script type='text/javascript'>document.getElementById('yoodooPost').submit();</script>";
		op+="</body></html>";
		var iframe=document.createElement("iframe");
		this.msdocFrame=iframe;
		$(iframe).css({width:1,height:1,border:'none',opacity:0});
		this.container.appendChild(iframe);
		var doc=(iframe.contentWindow)?iframe.contentWindow:(iframe.contentDocument.document?iframe.contentDocument.document:iframe.contentDocument);
		doc.document.open();
